...
<!--
 function doSubmitForm(url){
  ...
  function searchDocHD() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_VP_List.jsp";
  ...
  function showComment() {
     ...
     var sub = document.forms[0].elements("i_company");
  ...
  function approve_job() {
     if (!checkTickItem()) {
      ...
      pleaseWaiting2();  
     document.forms[0].mode.value="APPROVE_COMPANY";          
  ...
  function reject_job() {
      ...
      if (confirm("คุณแน่ใจว่าต้องการ Reject ใบงานที่เลือกนี้ ?")) {
         ...
         if (trim(document.forms[0].main_comment.value)=="") {
           ...
           pleaseWaiting2();  
          document.forms[0].mode.value="REJECT_COMPANY";          
          document.forms[0].action="<%=Constants.APP_PATH%>/SERV_VPConfServlet";
          document.forms[0].submit();
      }
  }  
...
<!--<input type="hidden" name="success_page" value="SERV_VP_List.jsp?d_payment=<%//=dPayment%>">-->
<input type="hidden" name="success_page" value="SERV_VP_List.jsp?<%=request.getQueryString()%>">
<input type="hidden" name="error_page" value="SERV_VP_List.jsp?error=1&d_payment=<%=dPayment%>">
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <%
        if (permission) {
            %>&nbsp;<!-- <input type="checkbox" name="main_check" onclick="checkAll(this,'main_check','i_company');"> --><%
        } else {
            %>&nbsp;<%
        }
    %>
   </td>
    <td width="30%" class="col_name" rowspan="2">บริษัท</td>
  ...
  %>
         ...
         <%
			...
			if(user.getUserWho().equals("A")){ //lee admin
			   sqlB.delete(0,sqlB.length());
		        ...
		        int line = 0;		     
		        sql.delete(0,sql.length());
		        sql.append(" select distinct d.n_company,d.i_company from lan:serv_payment b ")
		              .append(" left join lan:acxcompa d on d.i_company=substr(b.i_docno,1,2)  ")
		              .append(" ,lan:serv_dochd a where b.i_docno=a.i_docno and a.f_status='OPN' and b.f_itmstatus='800' ")
		              .append(condition)
		               .append(sqlB.toString())
		              .append(" order by d.i_company ");  		              
		        //System.out.println("SQL Main SERV_VP_LIST: "+sql.toString());     
				servlog.startLog(sql.toString());
		        rs = stmt.executeQuery(sql.toString());
				servlog.endLog();
				String iCompany = "";
                      ...
                      while (rs.next()) {
							...
							sql.append(" select b.i_docno,sum(b.q_wage_unit*b.z_wage_price) sum_wage, ")
							     ...
							     .append(" and a.i_company='"+iCompany+"' ")
							      ...
							      .append(sqlB.toString())
							      .append(" group by b.i_docno ");
				            //System.out.println("SQL333 : "+sql.toString());     
							servlog.startLog(sql.toString());
							rs1 = stmt1.executeQuery(sql.toString());
							servlog.endLog();
							while (rs1.next()) {
							...
							//System.out.println("SQL444 : "+sql.toString());     
							servlog.startLog(sql.toString());
							rs1 = stmt1.executeQuery(sql.toString());
							servlog.endLog();
							while (rs1.next()) {
					        ...
					        %>
							  <tr>
							    ...
							    <%
							        if (permission) {
							            %>
							            <input type="checkbox" name="i_company" value="<%=iCompany%>" onclick="checkAll(this,'main_check','i_company');">
							            <input type="hidden" name="<%=iCompany%>_name" value="<%=companyName%>">
							            <%
							        } else {
							            %>&nbsp;<%
							        }
							    ...
							    <td width="30%" class="dotline" align="left"><a href="#" onclick="javascript:doSubmitForm('SERV_VP_List2.jsp?i_company=<%=iCompany%>&d_payment=<%=dPayment%>');"><%=iCompany%> - <%=companyName%></a></td>
        ...
        %>           
  <tr>
    <td class="solidline ; item" align="center" colspan="3">รวม</td>
...
<%     System.out.println("-----SERV_VP_List.jsp : ");
	} catch (Exception e) {
		System.out.println("ERROR SERV_VP_List.jsp : " + e.getMessage());
