...
<FORM name="frmAddInfra" method="post" action="SERV_InfHome.jsp">
              ...
              <table border="0" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                  ...
                  <td height="22" width="31%" class="dotline01"> 
                    <select size="1" name="Project" class="box" style="width:250px" onChange="frmAddInfra.submit()">
...
%>                      
                    ...
                    <A HREF="javascript:frmAddInfra.submit()"><img border="0" src="images/bu_go.gif" align="absmiddle" width="40" height="22"></a> 
                  </td>
                </tr>
              </table>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	...
	<td class="item_tab3"></td>
            ...
            <td class="item_tab5i" style="width:200px" >
			&nbsp;<a href="SERV_Add_Infra.jsp">บันทึกค่าสาธารณะ&nbsp;<img border="0" src="images/i_pen.gif" align="absmiddle" width="14" height="16"></a>
			&nbsp;<a href="InitEdtInfraServlet?docNo=&Mode=A">บันทึกค่าสาธารณะแบ่งจ่าย&nbsp;<img border="0" src="images/i_pen.gif" align="absmiddle" width="14" height="16"></a>			
			&nbsp;<a href="InitEdtInfraServlet?docNo=&Mode=Y">บันทึกค่าสาธารณะย้อนหลัง&nbsp;<img border="0" src="images/i_pen.gif" align="absmiddle" width="14" height="16"></a>			
			</td>
	</tr>
</table>
<table border="0" width="100%" cellspacing="0" cellpadding="0">
    ...
    <td width="100%" class="frmL">
              <table border="0" width="100%" cellspacing="0" cellpadding="0">
                <tr> 
                  ...
                  <td class="col_name" width="13%">แปลง</td>
                  <td class="col_name" width="11%">วันที่แจ้ง</td>
                  <td class="col_name" width="35%">ผู้ชำระค่าสาธารณะ</td>
                  <td class="col_name" width="16%">จำนวนเงิน</td>
...
<%
	if (!betweenDate.equals("")) {
		...
		numberOfSorts = 0;
		sql.delete(0, sql.length());
		sql.append("SELECT COUNT(*) AS NUM_LOCK FROM lan:serv_infhd WHERE i_company = '"+comId+"' AND i_project = '"+projId+"' "+restrict+" AND d_start = '"+startDate+"' AND d_end = '"+endDate+"'");
		servlog.startLog(sql.toString());
		rs = stmt.executeQuery(sql.toString());
		servlog.endLog();
		sql.delete(0, sql.length());
		...
		i=0;
		sql.delete(0, sql.length());
		sql.append("SELECT i_docno, i_sort, i_lor, d_keyin, i_inf_custo, i_infra, n_custo, NVL(z_infra,0) AS INF_AMT, i_doc_status FROM lan:serv_infhd WHERE i_company = '"+comId+"' AND i_project = '"+projId+"' "+restrict+" AND d_start = '"+startDate+"' AND d_end = '"+endDate+"' ORDER BY i_sort");
		servlog.startLog(sql.toString());
		rs = stmt.executeQuery(sql.toString());
		servlog.endLog();
		sql.delete(0, sql.length());
			...
			while (rs.next() == true) {
                ...
                if (lineNumber >= 1) {
	                if (lineNumber <= MAX_LINE) {
						...
						targetPage = "InitEdtInfraServlet";
						if (status.equals("C") || status.equals("F")) {
							targetPage = "SERV_Disp_Infra.jsp";
						}
...
%>
                <tr> 
                  <td class="dotline" align="center" width="11%"><a href="/LHServ/<%=targetPage%>?docNo=<%=docNo%>"><%=docNo%></a></td>
                  <td class="dotline" align="center" width="13%"><%=sortId%></td>
                  <td class="dotline" align="center" width="11%"><%=DateUtil.ifxToThaiDateNoTime(doString.DisplayThai(rs.getString("D_KEYIN")))%></td>
                  <td class="dotline" width="35%"><%=doString.DisplayThai(custName)%></td>
                  <td class="dotline" align="right" width="16%"><%=doString.displayNumber("###,###,###.00", amount)%></td>
...
%>
                <tr> 
                  ...
                  <td class="dotline" align="right" width="16%">&nbsp;</td>
...
<%
			...
			while (true)
			{
				...
				pages++;
%>
			<A href='SERV_InfHome.jsp?<%=params%>&show=<%=show%>&selPage=<%=pages%>'><%if (selPage == pages) {%><b><%}%><%=pages%></b></A>&nbsp;&nbsp;
<%
				show += MAX_LINE;
				if (TotSort <= 0)
				{
...
<%
	...
	System.out.println("ERROR SERV_InfHome.jsp : " + sql.toString());
