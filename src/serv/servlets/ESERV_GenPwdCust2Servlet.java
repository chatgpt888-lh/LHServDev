 ...
 */
 ...
 public class ESERV_GenPwdCust2Servlet extends  DBServlet{
	    /* (non-Java-doc) @see javax.servlet.http.HttpServlet#HttpServlet() */
		public ESERV_GenPwdCust2Servlet() {
			super();
		}   
	        ...
	        try{	        	
				...
				if (rs.next()) {
					...
					sql.append(" SELECT DISTINCT a.i_company, a.i_project, a.n_project")
						...
						.append(" AND b.d_year in('"+cur_year1+"','"+cur_year+"')   ")
						//.append(cur_year)
		...
		protected void doGenerateAction(HttpServletRequest request, HttpServletResponse response,User user) throws ServletException, IOException{
	        ...
	        try{ 
	        	...
	        	if(intUpd!=-1){
	        		...
	        		String forward =  request.getContextPath()+"/save_ok.jsp?other_msg=&error=0&redirect_url=ESERV_GenPwdCust2Servlet?cmd=formLoad";		
	        		response.sendRedirect(forward);
	        	}else{
	        		//System.out.println("-------intsert fails");	
					String forward =  request.getContextPath()+"/save_ok.jsp?other_msg=&error=99&redirect_url=SERV_Home.jsp";			
					response.sendRedirect(forward);
	        	}	
	 			/********************************************************************/		
	 			//conn.commit();
