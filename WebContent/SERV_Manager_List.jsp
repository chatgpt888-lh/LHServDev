...
String jName = "SERV_Manager_List.jsp";
...
<!--
  ...
  function searchDocHD() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_Manager_List.jsp";
        ...
        <%
		     ...
		     //----================== Select Data from SERV_DOCHD ================----//
		        ...
		        sql.append(" select first ").append(endRow).append(" distinct d.bus_name,b.i_vendor ")
		              ...
		              .append(" ,lan:serv_dochd a where b.i_docno=a.i_docno and a.f_status='OPN' and b.f_itmstatus='600' ")
		              .append(condition)
		        ...
		        for (int i=0;i<maxRow;i++) {
                      if (rs.next()) {
                         if (i>=startRow && i<=endRow) {
							...
							//----====================== Count Job Form ==================----//
							sql.delete(0,sql.length());
							sql.append(" select count(*) from serv_payment b,serv_dochd a where b.f_itmstatus='600' ")
							...
							if (rs1.next()) {
							    ...
							    grandTotal += totalSum;
							    //sumCalMarkup += calMarkupPay;                 // edit by pay
							}
							rs1.close();
					   ...
					   //---=========================================================================----//      
					        ...
					        %>
							  <tr>
							    <td width="30%" class="dotline" align="left"><a href="SERV_Manager_Conf.jsp?i_vendor=<%=iVendor%>&sel_project=<%=selProj%>"><%=vendorName%> <%=markupPay%></a></td>
...
<%
		...
		System.out.println("ERROR SERV_Manager_List.jsp : " + e.getMessage());
