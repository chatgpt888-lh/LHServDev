...
String params = "Project="+comId+projId+"&chkMonth="+chkMonth+"&chkYear="+chkYear+"&seqNo="+chkupNo;
...
%>
		<table border="0" width="100%" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td class="item_tab2" width="200">บ้านโอน Check up ครั้งที่ <%=chkupNo%></td>
...
<%
	...
	if (rs.next() == true) {
		out.print(doString.DisplayThai(doString.checkString(rs.getString("N_PROJECT"))));
	}
	rs.close();
	rs=null;
%>								
			...
			<tr>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td width="6%" class="col_nameO"><a href="SERV_ViewChkLckLst.jsp?<%=params%>&order=i_lock, i_chkseq, i_status">แปลงขาย</a>
<%
...
%>						
						...
						<td width="9%" class="col_nameO"><a href="SERV_ViewChkLckLst.jsp?<%=params%>&order=i_status, i_chkseq, i_lock">สถานะ</a>
<%
...
%>												
						...
						<td width="9%" class="col_name">บ้านเลขที่</td>
						<td width="30%" class="col_name">ชื่อลูกค้า</td>
...
<%
	...
	int seqNo = 0;
	i=0;
	rsChkup = ustmt.executeQuery("SELECT * FROM lan:serv_chklock WHERE i_session = "+sessionId+" AND user_id = '"+userId+"' AND i_chkseq = "+chkupNo+" ORDER BY "+order);
	if (rsChkup != null) {
...
%>
					<tr>
						...
						<td width="9%" class="dotline" align="center"><%=doString.checkString(rsChkup.getString("I_HOUSE"))%>&nbsp;</td>
						<td width="30%" align="left" class="dotline"><%=doString.DisplayThai(rsChkup.getString("N_NAME"))%>&nbsp;</td>
...
<%
		...
		System.out.println("ERROR SERV_ViewChkLckLst.jsp : " + e.getMessage());
