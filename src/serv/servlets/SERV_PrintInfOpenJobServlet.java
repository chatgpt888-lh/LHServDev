...
public class SERV_PrintInfOpenJobServlet extends DBServlet  {
...
*/		  
		...
		table.addCell(cell);		  
		cell = setCellAttribute("เลขที่ใบสั่งซ่อม : "+iDocNo,microssfont_MED,"L","M",rightColumnWidth,0);
		table.addCell(cell);
		...
		table.addCell(cell);
		cell = setCellAttribute("ชื่อผู้รับผิดชอบงานซ่อม : "+doString.MS874ToUnicode(inFormEmp),microssfont_MED,"L","M",leftColumnWidth,0);
		table.addCell(cell);		  
		...
		if(!Prj_Condo.equals("Y")){	
			...
			cell = setCellAttribute("ประเภท",microssfont_MINI,"C","M",itmtypeWidth,13);
			cell.setFixedHeight(22);
			table.addCell(cell);
			...
			cell = setCellAttribute("ประมาณ",microssfont_MED,"C","M",estWidth,13);
			cell.setFixedHeight(22);
			table.addCell(cell);
			...
			cell.setFixedHeight(22);
			table.addCell(cell);
			cell = setCellAttribute("งาน",microssfont_MED,"C","M",itmtypeWidth,14);
			cell.setFixedHeight(22);
			table.addCell(cell);
			...
			cell.setFixedHeight(22);
			table.addCell(cell);
			cell = setCellAttribute("ค่าซ่อม",microssfont_MED,"C","M",estWidth,14);
			cell.setFixedHeight(22);
			table.addCell(cell);
	...
	public void genCommonFooter(PdfPTable table,String vendorSign,String Prj_Condo,String n_service) throws Exception {
		...
		table.addCell(cell);
		cell = setCellAttribute("ลงชื่อเจ้าหน้าที่ผู้รับผิดชอบ",microssfont_MED_BOLD,"C","M",50,8);
		table.addCell(cell);
		...
		for (int loop=0;loop<vendorList.size();loop++) {
			    ...
			    while (line<vendorItem.size()) {
			    	...
			    	for (int i=(maximumLine*page);i<vendorItem.size();i++) {
			    		    ...
			    		    String boq = (String) data.get("boq");
			    		    String itemDesc = (String) data.get("item_desc");
			    		    String remarkDiff = "";
			    		    if (boq.equals("B")) {
			    		    	if (itemDesc.indexOf("*")==0) remarkDiff = "*(B) ";
			    		    } else {
			    		    ...
			    		    String itemType = (String) data.get("item_type");
							...
							cell = setCellAttribute(doString.MS874ToUnicode(itemType),microssfont_MINI,"C","M",itmtypeWidth,15);
							cell.setFixedHeight(25);
							table.addCell(cell);
							...
							double est_amnt = Double.parseDouble(doString.checkString((String) data.get("est_amnt"),"0"));
							double payAmnt = Double.parseDouble(doString.checkString((String) data.get("pay_amnt"),"0"));
							sumEst += est_amnt;
							sumWage += (qWage*zWage);
							sumGoods += (qGoods*zGoods);
							sumPayAmnt += payAmnt;
							cell = setCellAttribute(format.format(zWage),microssfont_MINI,"R","M",qWageWidth,15);
							cell.setFixedHeight(25);
							table.addCell(cell);
							cell = setCellAttribute(format.format(qWage),microssfont_MINI,"R","M",zWageWidth,15);
							cell.setFixedHeight(25);
							table.addCell(cell);
							cell = setCellAttribute(format.format((qWage*zWage)),microssfont_MINI,"R","M",tWageWidth,15);
							cell.setFixedHeight(25);
							table.addCell(cell);
							cell = setCellAttribute(format.format(zGoods),microssfont_MINI,"R","M",qGoodsWidth,15);
							cell.setFixedHeight(25);
							table.addCell(cell);
							cell = setCellAttribute(format.format(qGoods),microssfont_MINI,"R","M",zGoodsWidth,15);
							cell.setFixedHeight(25);
							table.addCell(cell);
							...
							cell = setCellAttribute(format.format(est_amnt),microssfont_MINI,"R","M",estWidth,15);
							cell.setFixedHeight(25);
							table.addCell(cell);								
			    	...
			    	if (itemLine<=maximumLine) {
			    		printFooter = true;
						cell = setCellAttribute("รวม    ",microssfont_MINI,"L","M",numWidth+itemWidth+countWidth+itmtypeWidth,15);
						...
						cell = setCellAttribute(format.format(sumEst),microssfont_MINI,"R","M",estWidth,15);
						cell.setFixedHeight(20);
						table.addCell(cell);
		 ...
		 try {
			...
			//---------------- Check Project is Condo ---------------					
				String emp_serv = "", name_serv = "", Prj_Condo = "", n_service = "";	
					...
					Prj_Condo = "N";
					sql.delete(0, sql.length());
					sql.append("select n_service from lan:serv_prjdt ")
			...
			while (rs.next()) {
				...
				if (printTarget.equalsIgnoreCase("PAYMENT")) {
					...
					sql.delete(0,sql.length());
					sql.append("select * from lan:serv_infdocdt where i_itmjob='").append(iItmJob).append("' ")
