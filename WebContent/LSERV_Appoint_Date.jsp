...
function doGO(){
      if( document.forms[0].projectDDL.value=='' ){
         ...
         doucument.forms[0].projectDDL.select();
		  ...
		  document.forms[0].action="<%=request.getContextPath()%>/LSERV_AppointDateServlet?cmd=loadTime";
...
function doSubmit(){
     if(trim(document.forms[0].MrnList.value)=="" || document.forms[0].MrnList.value.length == 0){
        ...
        return;
     	...
     	alert("กรุณาเลือกเวลานัดเข้าตรวจสอบด้วย");
     	return;
     }else{  
	     //validate from client side
		 document.forms[0].action="<%=request.getContextPath()%>/LSERV_AppointDateServlet?cmd=formSubmit";
		 document.forms[0].submit();
	 }
}

  function doONxOFF(param,i){
	   if(true==param){
	       document.getElementById("sysDDL"+i).disabled=false;
	   }else{
	       document.getElementById("sysDDL"+i).disabled=true;
    }
  }
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td class="item_tab2" width="280">(สำหรับ Line Service)กำหนดวันนัดเข้าตรวจสอบรายการซ่อม</td>
			...
			<tr>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					...
					<%
					 ...
					 %>
						<select name="projectDDL" class="box2" style='width:250' size='1'  > 
			...
			<tr>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr>
						...
						<table border="0" width="100%" cellspacing="0" cellpadding="0" >
							<tr>
								<td class="col_name" width="80%">***หมายเหตุ </td>
							</tr>
							<tr>
								<td class="dotline ; item" width="80%" align="center">
							 	<TEXTAREA name="commentTxt" cols=35 rows=6 wrap=physical style="font-family: MS Sans Serif;font-size:10pt" readonly class="box7">
								</TEXTAREA>
								</td>
							</tr>
						</table>
						...
						<td width="100%" class="frmL" valign="top">
							<table border="0" width="100%" cellspacing="0" cellpadding="0">
								<tr>
									...
									<td class="col_name" width="40%">เวลานัดหมายของระบบ</td>
								</tr>
						...
						<%
								...
								while(it.hasNext()){  								
									...
									<input type="checkbox" name="timeChecked" id="chk<%=i %>" value="<%=arrList2.get(0).toString() %>" checked 
 										onClick="JavaScript:doONxOFF(this.checked,<%=i %>);"></td>
									<td class="dotline ; item" width="50%"><%=arrList2.get(0).toString() %></td>
									<td class="dotline ; item" width="40%">
										<select size="1" name="<%=arrList2.get(0).toString() %>" id="sysDDL<%=i%>" class="box2" style="width:250px">
									         <%-- <option>--- กรุณาเลือกเวลานัดหมายของระบบ ---</option>--%>
									        <option value="03" >03 Line Service</option>
									        <option value="02" disabled>02 Service Center</option>
										    <option value="01" disabled>01 e-Service</option>
										    <%-- <option value="03">วันหยุด</option>
										    <option value="04">วันประชุม</option>
										    --%>
								       </select>
									</td>
									</tr>
