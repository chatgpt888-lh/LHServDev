...
<!--
  ...
  function searchDocHD() {
     ...
     document.forms[0].action="/LHServ/SERV_INFManager_List.jsp";
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td width="30%" class="col_name" rowspan="2">ผู้รับเหมาซ่อม</td>
                  <td width="10%" class="col_name" rowspan="2">จำนวนใบสั่งซ่อม</td>
                      ...
                      if (rs.next()) {
						...
						if (selProj.trim().length()>0 && iVendor.trim().length()>0) {
							...
							if (itmType.equals("01")) {
								...
								pAddPay = rs1.getDouble("p_pub_pay");
					   ...
					   //---=========================================================================----//      
					        %>
					  <tr>
					    <td width="30%" class="dotline" align="left"><a href="SERV_INFManager_Conf.jsp?itmType=<%=itmType%>&i_vendor=<%=iVendor%>&sel_project=<%=selProj%>"><%=vendorName%> <%=markupPay%></a></td>
...
<%
		...
		System.out.println("ERROR SERV_INFManager_List.jsp : " + e.getMessage());
