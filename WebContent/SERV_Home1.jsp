		...
		Statement stmt = null;
		 ...
		 try {
			...
			if (userWho.equalsIgnoreCase(Constants.PERMISSION_VENDOR)) {
			 ...
			 html.append("<option value='ALL_PROJ' "+(value.equalsIgnoreCase("ALL_PROJ") ? "selected" : "")+">"+Constants.LISTBOX_ALLPROJECT_LABEL+"</option>");
			 html.append("</select>");
			 //----=====================================================----//
...
System.out.println("Start "+jName);
System.out.println("Sel Project = "+selProj);
/*
   if  (selProj.length()==0) {
   ...
   //-----====================== Search BOQ Data ================================------//
	...
	try {
       ...
       String condition = "";	
				   ...
				   condition += " and a.i_docno='NOPROEJCT' ";
					...
					if (checkAllPermission<=0) {
					   ...
					   condition += " and x.com_id = a.i_company and x.proj_id = a.i_project) ";
						...
						//---- user view all project , no condition -----//
						condition += " and a.i_docno='NOPROEJCT' ";
					}
			   }
        ...
        sql.delete(0,sql.length());
        sql.append(" select * from serv_payschd where d_change>=today order by d_payment ");
		servlog.startLog(sql.toString());
        rs = stmt.executeQuery(sql.toString());
		servlog.endLog();
        int cnt = 1;
		...
		if (!user.getUserWho().equalsIgnoreCase(Constants.PERMISSION_VENDOR)) {
			...
			sql.delete(0,sql.length());
			sql.append(" select count(*) from serv_dochd a where a.f_status='OPN' and a.i_doc_type='I' and a.c_desc != 'Checkup Program' ")
				  .append(condition);
System.out.println(condition);			  
			...
			sql.delete(0,sql.length());
			sql.append(" select distinct a.i_docno, b.f_itmstatus from serv_dochd a,serv_docdt b ")
...
System.out.println(sql.toString());
        ...
        sql.append(" select count(distinct b.i_docno) as cnt ,b.f_itmstatus,a.f_reject,a.d_reject,b.d_payment ")
			  .append(" from lan:serv_infdochd a,lan:serv_infpayment b where a.f_status in ('OPN','CLS') and a.i_doc_type='J' ")
			  .append(" and b.f_itmstatus!='CAN' and b.i_docno=a.i_docno "+condition+" ")
			  .append(" and (b.d_payment='"+currentPaymentDate+"' or b.d_payment='"+nextPaymentDate+"') ")
			  .append(" group by b.f_itmstatus,a.f_reject,a.d_reject,b.d_payment ");
System.out.println("INF=>"+sql.toString());
		rs = stmt.executeQuery(sql.toString());
		while (rs.next()) {
			itemStatus = doString.checkString(rs.getString("f_itmstatus"),"");
			fReject = doString.checkString(rs.getString("f_reject"),"");
...
<!--
   ...
   function queryProject() {
       document.forms[0].action = "SERV_Home1.jsp?search=y";
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td width="100%" align="center" class="BD">
	  ...
	  <%
	   if (!user.getUserWho().equals("V") && !user.getUserWho().equals("G")) {
...
<%
	completeWait=0;
	sql.delete(0,sql.length());
	sql.append("SELECT DISTINCT a.i_docno FROM lan:serv_infdochd a, serv_infdocdt b ")
...
<%
		...
		System.out.println("ERROR SERV_Home1.jsp : " + e.getMessage());
