...
<%
	...
	String jName = "SERV_Conf_RetReten.jsp";
   ...
   if (checkUser!=null) {
	   userWho = checkUser.getUserWho();
   } else {
	   userWho = doString.checkString(request.getParameter("user_who"),"");
   }
	...
	//-----======== Get Reten Data ==========----//
	sql.delete(0,sql.length());
	sql.append(" select b.i_company||b.i_project||' | '||b.n_project as project_name , c.n_desc , e.c_apprv , s.n_desc as n_paybnk , ")
	      ...
	      .append(" left join lan:serv_apprv e on e.i_docno=a.i_docno and e.i_doc_status=a.i_doc_status ")
	      //----- 2022-06-30 , add payin query ------//
	      .append(" left join lan:lhpay_std s on s.i_type='R' and s.i_key1=a.i_paybnk ")
	      //-----------------------------------------//	  	      
	      .append(" where a.i_doc_status in ('W','G') and a.z_reten=a.z_recv_reten and a.i_staff_payback is not null ")
	...
	Vector approvePositionList = new Vector();
        ...
        //-----========== Get Approve List  ============-----//
	if (
	    userWho.equalsIgnoreCase("A") ||  // can delete this line
	    userWho.equalsIgnoreCase("M")) {
		//|| userWho.equalsIgnoreCase("Z")) {
		sql.delete(0,sql.length());
		/*
		sql.append(" select i_employ from lan:useracl where user_id in ( ")
		      ...
		      .append(" and user_who='Z' and user_acl='S' ");  // old is P
		*/
		sql.append(" select i_employ from lan:useracl where user_id in ( ")
		      .append(" select user_id from lan:serv_pstaff where ")
		      .append(" com_id='").append(iCompany).append("' and ")
			  .append(" (proj_id='").append(iProject).append("' or proj_id='ALL') ")
		      .append(" ) and (user_who='Z' or (user_who='C' and user_id='prapat')) and user_acl='S' ");
	...
	if (userWho.equalsIgnoreCase("F")) {
		sql.delete(0,sql.length());
		sql.append(" select i_employ from docflow:acemploy where user_who='R' and user_acl='S' ");
		servlog.startLog(sql.toString());
		rs = stmt.executeQuery(sql.toString());
		servlog.endLog();
		while (rs.next()) {
...
function approveRetReten() {
     ...
     if (confirm("คุณแน่ใจว่าต้องการ Approve ใบรายการนี้  ?")) {	
        document.forms[0].approve_flag.value="G";
...
function routebackRetReten() {
     ...
     if (confirm("คุณแน่ใจว่าต้องการ Route Back ใบรายการนี้ ?")) {	
        document.forms[0].approve_flag.value="R";
        document.forms[0].submit();
     }
}
</script>
...
<form action="<%=Constants.APP_PATH%>/SERV_Conf_RetRetenServlet" method="post">
...
<br style="font-size:10pt">
            ...
            <table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr>
                <td class="item_tab1"><img border="0" src="images/i_i.gif" align="absmiddle" width="20" height="20"></td>
                <td class="item_tab2" width="200">Comment &nbsp; ผู้ตรวจเอกสาร</td>
                <td class="item_tab3"></td>
                <td>&nbsp;</td>                
              </tr>
            </table>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td width="100%" class="frmLRpad01"><textarea rows="5" name="c_apprv" class="box" style="width:100%" cols="20" <%=(iDocStatus.equals("W")) ? "" : " disabled"%>><%=cApprv%></textarea></td>
  </tr>
</table>
...
<table cellspacing="0" cellpadding="0" width="100%">
					...
					sql.delete(0,sql.length());
					sql.append(" select * from lan:serv_apprv where i_docno='").append(iDocNo).append("' ")
					      .append(" and i_doc_status='W' ");
					servlog.startLog(sql.toString());
					rs = stmt.executeQuery(sql.toString());
					servlog.endLog();
					if (rs.next()) {
					   String iEmploy = doString.checkString(rs.getString("i_apprv"),"");
					   String details[] = getEmployeeDetails(stmt1,iEmploy, servlog);
					   out.println("<font color='#000096'>"+doString.DisplayThai(details[1])+"</font> &nbsp; ( "+doString.DisplayThai(details[2])+" )");
					} 
					rs.close();
				     }
				    %>
...
<table cellspacing="0" cellpadding="0" width="100%">
				    ...
				    if ((iDocStatus.equals("W"))) { 
					    %>	
						    <font color="#000096">
						    <select size="1" name="i_cur_apprv" class="box" style="width:350px">
					    ...
					    String nextApprv[] = getEmployeeDetails(stmt1,iCurApprove, servlog);
					    out.println("<font color='#000096'>"+doString.DisplayThai(nextApprv[1])+"</font> &nbsp; ( "+doString.DisplayThai(nextApprv[2])+" )");
				    }				    
				    %>	
				    </td>
                                  </tr>
        ...
        <table border="0" width="100%" cellspacing="0" cellpadding="0" height="30">
          <tr>
		    ...
		    <%
		      if ((iDocStatus.equals("W"))) {
		    %>
		    <img border="0" src="images/act_approve.gif" onclick="approveRetReten();"
				onmouseout=nereidFade(this,70,50,5)
				onmouseover=nereidFade(this,100,50,5)
				style="FILTER: alpha(opacity=70);cursor:hand;" width="70" height="27">&nbsp;&nbsp;
		    <img border="0" src="images/act_routeback.gif" onclick="routebackRetReten();"
				onmouseout=nereidFade(this,70,50,5)
				onmouseover=nereidFade(this,100,50,5)
				style="FILTER: alpha(opacity=70);cursor:hand;" width="70" height="27">
		    <%
