...
<!--
  ...
  function searchDocHD() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_CONVP_List.jsp";
  ...
  function approve_job() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_CONVPConfServlet";
     document.forms[0].submit();
  }  
...
<input type="hidden" name="success_page" value="SERV_CONVP_List.jsp?itmType=<%=itmType%>&d_payment=<%=dPayment%>">
<input type="hidden" name="error_page" value="SERV_CONVP_List.jsp?error=1&itmType=<%=itmType%>&d_payment=<%=dPayment%>">
...
<br style="font-size:10pt">
            ...
            <table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td class="item_tab2" width="200">รายละเอียดงวดงาน</td>
  ...
  %>
         ...
         <%
		     ...
		     //----================== Select Data from SERV_DOCHD ================----//   
		        int line = 0;		     
		        sql.delete(0,sql.length());
		        sql.append(" select distinct d.n_company,d.i_company from lan:serv_infpayment b ")
				...
				rs = stmt.executeQuery(sql.toString());
                      ...
                      while (rs.next()) {
				            ...
				            String companyName = doString.checkString(doString.DisplayThai(rs.getString("n_company")),"");			            
					        ...
					        %>
							  <tr>
							    ...
							    <td width="30%" class="dotline" align="left"><a href="SERV_CONVP_List2.jsp?itmType=<%=itmType%>&i_company=<%=iCompany%>&d_payment=<%=dPayment%>"><%=iCompany%> - <%=companyName%></a></td>
...
<%
		...
		System.out.println("ERROR SERV_CONVP_List.jsp : " + e.getMessage());
