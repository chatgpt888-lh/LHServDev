...
String jName = "SERV_Conf_SRecevChq.jsp";
ServLog servlog = new ServLog(sessionId, userId, jName);
   ...
   String condition = "";
	...
	sql.append(" select b.i_company||b.i_project||' | '||b.n_project as project_name , c.n_desc , e.c_apprv , f.c_apprv as  c_prev_apprv , ")
	      .append(" trim(d.n_prename_th)||trim(d.n_nemploy_th)||' '||trim(d.n_semploy_th) as emp_name , ")
		  .append(" spl_fn_chq_no(a.i_company,a.i_pvno) as fnc_i_cheque , ")
		  .append(" spl_fn_chqamt(a.i_company,a.i_pvno) as fnc_z_cheque , ")
		  .append(" a.* from lan:serv_rethd a ")
	      ...
	      .append(" where a.i_doc_status in ('A','K') and a.z_reten=a.z_recv_reten and a.i_staff_payback is not null ")
	      .append(" and i_pvno is not null and a.i_docno='").append(docNo).append("' ");
	...
	while (rs.next()) {
             ...
             retCustType = doString.DisplayThai(doString.checkString(rs.getString("i_ret_custo"),""));
             iReten = doString.checkString(rs.getString("i_reten"),"");
             retCustTel = doString.checkString(rs.getString("i_reten_tel"),"");
			...
			//----- Get Cheque Details ------//
	         iCheque = doString.checkString(doString.DisplayThai(rs.getString("fnc_i_cheque")),"");
			 if (iCheque.trim().length()>0) {
				 iChqBank = doString.checkString(rs.getString("i_pv_bank"),"");
				 iChqBran = doString.checkString(rs.getString("i_pv_bran"),"");
				 zCheque =  rs.getDouble("fnc_z_cheque");

		        est = Calendar.getInstance(Locale.ENGLISH);
				tmp = rs.getTimestamp("d_pvno");
				if (tmp!=null) {
				 est.setTime(tmp);
				 dPvNo =  getDateFromCalendar(est);
				} else {
					 dPvNo = "";
				}
			 }
	    ...
	    est = Calendar.getInstance(Locale.ENGLISH);
   	    tmp = rs.getTimestamp("d_reten_payback");
	    if (tmp!=null) {
		 est.setTime(tmp);
	...
	rs.close();
        ...
        if (retCustType.equals("1")) {
   	    sql.append(" select trim(n_prename)||trim(n_ncustomer)||' '||trim(n_scustomer) as cust_name , ' ' as i_tel ")
	...
	if (rs.next()) {
		...
		if (!retCustType.equals("1")) {
			retCustTel = doString.checkString(rs.getString("i_tel"),"");
		}
	}
	rs.close();
...
function confirmCheque() {
     if (confirm("คุณแน่ใจว่าต้องการ Confirm เช็คคืนใบนี้  ?")) {
        document.forms[0].action_flag.value="K";
...
<form action="<%=Constants.APP_PATH%>/SERV_ManageChequeServlet" method="post">
...
<input type="hidden" name="i_cheque" value="<%=doString.checkString(iCheque,"")%>">
<input type="hidden" name="z_cheque" value="<%=format.format(zCheque)%>">
<input type="hidden" name="action_flag" value="K">
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td width="16%" height="22" class="item ; dotline01" valign="top">ผู้รับเงินตามเช็ค
      :</td>
    <td width="38%" height="22" class="dotline01" valign="top">
	<%=doString.checkString(retCustName,"-")%> &nbsp; 
	<font color="#FF6400">(<%=doString.checkString(retCustTel,"-")%>)</font>
	</td>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td width="100%" class="frmLRpad01"><textarea rows="5" name="c_apprv" disabled class="box" style="width:100%" cols="20"><%=cApprv%></textarea></td>
  </tr>
</table>
...
<br style="font-size:10pt">
            ...
            <table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr>
                <td class="item_tab1"><img border="0" src="images/i_i.gif" align="absmiddle" width="20" height="20"></td>
                <td class="item_tab2" width="200">รายละเอียดหลักฐานการคืนเงิน</td>
                <td class="item_tab3"></td>
                <td>&nbsp;</td>
              </tr>
            </table>
...
<%
	//-----======== Get Reten Data ==========----//
	sql.delete(0,sql.length());
	sql.append("select * from lan:acxfinan where ( ")
	      .append(" (i_finance='").append(iChqBank).append("' and i_branch is null) or ")
	      .append(" (i_finance='").append(iChqBank).append("' and i_branch='").append(iChqBran).append("') ) ");
	servlog.startLog(sql.toString());
	rs = stmt.executeQuery(sql.toString());
	servlog.endLog();
	while (rs.next()) {
            String i_branch = doString.checkString(rs.getString("i_branch"),"");
	    if (i_branch.trim().length()<=0) {
	        bankName = doString.checkString(doString.DisplayThai(rs.getString("n_finance")),""); 
	    } else {
	        branchName = doString.checkString(doString.DisplayThai(rs.getString("n_finance")),""); 
	    }
	}
	rs.close();
%>
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td class="item ; dotline01" height="22" width="16%">เลขที่เช็คคืน
      :</td>
    <td height="22" width="38%" class="dotline01"><%=doString.checkString(iCheque,"-")%></td>
    <td height="22" class="item ; dotline01" width="16%">ลงวันที่  :</td>
    <td height="22" width="30%" class="dotline01"><%=doString.checkString(dPvNo,"-")%></td>
  </tr>
  <tr>
    <td class="item ; dotline01" height="22" width="16%">ธนาคาร :</td>
    <td height="22" width="38%" class="dotline01"><%=doString.checkString(bankName,"-")%></td>
    <td height="22" class="item ; dotline01" width="16%">สาขา :</td>
    <td height="22" width="30%" class="dotline01"><%=doString.checkString(branchName,"-")%></td>
  </tr>
  <tr>
    <td class="item ; dotline01" height="22" width="16%">จำนวนเงิน :</td>
    <td height="22" width="38%" class="dotline01"><%=format.format(zCheque)%>&nbsp; บาท</td>
        ...
        <table border="0" width="100%" cellspacing="0" cellpadding="0" height="30">
          <tr>
            ...
            <td width="75" class="act_tab2">
             <%
			   if (iDocStatus.equals("A") && iCheque.trim().length()>0) {
					 %>		
						<img border="0" src="images/act_confirmCheque.gif"  onclick="confirmCheque();"
...
<TABLE border=0 cellspacing=0 cellpadding=0 width="100%">
  ...
  <br><img src="images/copyright.gif" width="475" height="26"></td></tr>
             ...
             <%
			   if (iCheque.trim().length()<=0) {
					 %>		
						<script>alert("รอทำเช็คคืน , กรุณาติดต่อฝ่ายบัญชี");</script>
					<%
			  }
			%>
