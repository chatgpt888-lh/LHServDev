 ...
 * @author
 */
public class SERV_BOQServlet extends DBServlet  {
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		String successPage = "SERV_BOQ01.jsp";
		String errorPage = "SERV_BOQ02.jsp?i_group="+iGroup+"&i_type="+iType+"&i_seq="+iSeq+"&i_itmjob="+iItm+"&mode="+mode;
		...
		ResultSet rs = null;
		 ...
		 try {
			...
			if (mode.equalsIgnoreCase("ADD")) {
				...
				sql.append(" select count(*) as cnt from lan:serv_boq where ")
				...
				if (cnt==0) {
					...
					sql.delete(0,sql.length());
					sql.append("insert into lan:serv_boq (i_group,i_type,i_seq,i_itmjob,n_itmjob,z_wage_unit,z_good_unit,n_count,d_keyin")
						  .append(" ) values ( ")
					...
					for (int x = 0; x < Integer.parseInt(no); x++) {						
						      
						String i_cause = doString.checkString(req.getParameter("cau"+x),"-");
						
						//System.out.println("i_cause=="+i_cause);
						
						if (!i_cause.equals("-")) {
						
								sql.delete(0,sql.length());
								sql.append("insert into lan:serv_cause (i_itmjob,i_cause ")
									  .append(" ) values ( ")
									  .append(" '").append(iItm).append("' , ") 		
									  .append(" '").append(i_cause).append("' ")
									  .append(" ) "); 	
			...
			// ----=================== Start Edit Mode ==================-----//	
				 ...
				 iItm = doString.checkString(req.getParameter("i_itmjob"),""); 
					...
					sql.append("update lan:serv_boq  set ")
					...
					sql.delete(0,sql.length());
					sql.append("delete from lan:serv_cause ")
					   .append(" where i_itmjob='").append(iItm).append("' ");								  
					stmt.executeUpdate(sql.toString());
					...
					for (int x = 0; x < Integer.parseInt(no); x++) {						
						...
						System.out.println("edit i_cause=="+i_cause);
			...
			//----======== Delete Mode , Insert Query =========----//
				 ...
				 successPage = Constants.APP_PATH+"/SERV_BOQ01.jsp";	 
				 savePage = Constants.APP_PATH+"/SERV_BOQ01.jsp";	 
	 			 errorPage = Constants.APP_PATH+"/SERV_BOQ01.jsp&error=1";	
	 			 otherMsg="";
	 			 ...
	 			 if (delid!=null) {
	 			 	 ...
	 			 	 for (int i=0;i<delid.length;i++) {
	 			 	 	    ...
	 			 	 	    sql.delete(0,sql.length());
							sql.append("delete from lan:serv_boq ")
