...
String jName = "SERV_Report12.jsp";
...
try {
	...
	rs.close();	
			maxYear = cur_year;
			minY = Integer.parseInt(cur_year)-3;
...
String A_StartM = doString.checkString(request.getParameter("A_StartM"),maxMonth);
String A_StartY = doString.checkString(request.getParameter("A_StartY"),maxYear);
String A_EndM = doString.checkString(request.getParameter("A_EndM"),maxMonth);
String A_EndY = doString.checkString(request.getParameter("A_EndY"),maxYear);

String B_StartM = doString.checkString(request.getParameter("B_StartM"),maxMonth);
String B_StartY = doString.checkString(request.getParameter("B_StartY"),maxYear);
String B_EndM = doString.checkString(request.getParameter("B_EndM"),maxMonth);
String B_EndY = doString.checkString(request.getParameter("B_EndY"),maxYear);
...
/*	 if (document.forms[0].A_StartM.value=="") {
	 ...
	 */
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_Report12_1.jsp";
...
<FORM ACTION="SERV_Report12.jsp" METHOD="POST" name="frm">
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
 <tr>
    ...
    <td width="80%" class="dotline01">
		...
		<%=common.genYearListbox("A_StartY",A_StartY," class='box' ",minY,5)%>		
		&nbsp;&nbsp; ถึง&nbsp;&nbsp;
		<%=common.genMonthListbox("A_EndM",A_EndM," class='box' ")%>&nbsp;
		<%=common.genYearListbox("A_EndY",A_EndY," class='box' ",minY,5)%>
	</td>
 </tr>
 <tr>
    ...
    <td width="80%" class="dotline01">
		...
		<%=common.genYearListbox("B_StartY",B_StartY," class='box' ",minY,5)%> &nbsp;&nbsp; ถึง&nbsp;&nbsp;
		<%=common.genMonthListbox("B_EndM",B_EndM," class='box' ")%>&nbsp;
		<%=common.genYearListbox("B_EndY",B_EndY," class='box' ",minY,5)%>
	</td>
 </tr>
 <tr>
...
%>
  ...
  <tr>
    <td width="20%" height="22" class="item ; dotline01" style="padding-left:27px">รายงานที่ต้องการ : </td>
    <td width="80%" class="dotline01"><select size="1" name="repdisplay" class="box" style="width:320px">
	<option value="01" <% if (repdisplay.equals("01")) { out.println("Selected"); } %>>ตามแบบบ้าน</option>
	<option value="02" <% if (repdisplay.equals("02")) { out.println("Selected"); } %>>ตามประเภทบ้าน</option>
	<option value="03" <% if (repdisplay.equals("03")) { out.println("Selected"); } %>>ตามผู้รับเหมา</option>
</select></td>
  </tr>
</table>
...
<%
	...
	System.out.println("ERROR SERV_Report12.jsp : " + e.getMessage());
