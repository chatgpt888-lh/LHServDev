...
<%
	User user = null;
	String actionPage = "iupload_progress.jsp";
	String userId = "";
	if (session != null) {
	...
	/*if (userId.equals("lee")) {
		actionPage = "UploadServlet";
	}*/
   String sessionId = request.getParameter("session_id");
   if (sessionId==null) sessionId = "";
   session.setAttribute("session_upload_id",sessionId);
   String itmNo = request.getParameter("itmNo");
   String keyFile = request.getParameter("key_file");
	session.setAttribute("itemNo",itmNo);
	session.setAttribute("key_file",keyFile);
...
<script src='resources/js/iupload.js'> </script>
	...
	for (var idx=1;idx<=1;idx++) {
			...
			if (file != null) {
				...
				if (fileName.lastIndexOf(".")>0) {
					...
					if (ext != "JPG") {
						alert(" ไฟล์ที่ upload ต้องเป็น jpg เท่านั้น !! ");
						return false;
					}
				} else {
					alert(" ไฟล์ที่ upload ต้องเป็น jpg เท่านั้น !! ");
...
<!--FORM action="iupload_progress.jsp" enctype="multipart/form-data" method="post" onsubmit="if (checkFileExt('1') &&checkFileExt('2')) startProgress(); else return false; "-->

<FORM action="<%=actionPage%>" enctype="multipart/form-data" method="post" onsubmit="startupload();">
...
<TABLE BORDER='0' CELLPADDING='5' CELLSPACING='2' width="390px" height="100px" valign="top">
  ...
  <TR>
    <TD  width="380px"  height="100px" bgColor="#DCF0FF"><FONT face="MS Sans Serif" color="#0032FF" valign="top" align="center" >
		<center>
		<table>
			<tr><td><nobr>ไฟล์แนบ : </nobr></td><td><input type="file" id="file1" name="file" class="box" style="width:250px;"  /></td></tr>
		</table>
        <br/>
