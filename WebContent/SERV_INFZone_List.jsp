...
<!--
  ...
  var old_project = "<%=selProj%>";
  var tmpComment = new Array(); 

  function searchDocHD() {
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_INFZone_List.jsp";
  ...
  function showComment() {
     ...
     var sub = document.forms[0].elements("i_vendor");
  ...
  function approve_job() {
	 ...
	 if (document.forms[0].sel_project.value!=old_project) {
		 alert("มีการเปลี่ยนโครงการ , กรุณากด GO เพื่อดึงข้อมูลใหม่อีกครั้ง !! ");
		 return false;
	 }
     
     document.forms[0].mode.value="APPROVE_VENDOR";          
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_InfZoneConfServlet";
     document.forms[0].submit();
  }  
  ...
  function reject_job() {
      ...
      if (confirm("คุณแน่ใจว่าต้องการ Reject ใบงานที่เลือกนี้ ?")) {
        ...
        document.forms[0].mode.value="REJECT_VENDOR";          
        document.forms[0].action="<%=Constants.APP_PATH%>/SERV_InfZoneConfServlet";
        document.forms[0].submit();
      }
  }  
...
<input type="hidden" name="success_page" value="SERV_INFZone_List.jsp?itmType=<%=itmType%>&d_payment=<%=dPayment%>">
<input type="hidden" name="error_page" value="SERV_INFZone_List.jsp?error=1&itmType=<%=itmType%>&d_payment=<%=dPayment%>">
...
<br style="font-size:10pt">
            ...
            <td class="item_tab2" width="200">รายละเอียดงานสั่งซ่อม</td>
                <td class="item_tab3"></td>
                <td >&nbsp;</td>
              </tr>
            </table>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <%
        if (permission) {
            %><input type="checkbox" name="main_check" onclick="checkAll(this,'main_check','i_vendor');"><%
        } else {
            %>&nbsp;<%
        }
    %>    
    </td>
    <td width="30%" class="col_name" rowspan="2">ผู้รับเหมาซ่อม</td>
    <td width="5%" class="col_name" rowspan="2">จำนวน<br>
                    ใบสั่งซ่อม</td>
        ...
        <%
			...
			if(user.getUserWho().equals("A")){ //lee admin
		     ...
		     //----================== Select Data from SERV_DOCHD ================----//   
		        int line = 0;		     
		        sql.delete(0,sql.length());
		        sql.append(" select distinct d.bus_name,b.i_vendor ")
		              .append(" from lan:serv_infpayment b ")
		              .append(" left join lan:stpvendr d on d.vend_code=b.i_vendor ")
		              .append(" ,lan:serv_infdochd a where b.i_docno=a.i_docno and a.f_status='OPN' and b.f_itmstatus='700' ")					  .append(team_condition)
		              .append(condition)
		        ...
		        rs = stmt.executeQuery(sql.toString());
                      ...
                      while (rs.next()) {
						   ...
						   //---=========================================================================----//      
							...
							//----====================== Count Job Form ==================----//
							sql.delete(0,sql.length());
							sql.append(" select count(*) from lan:serv_infpayment b,lan:serv_infdochd a where b.f_itmstatus='700' ")
							...
							sql.append(" select b.i_vendor,sum(b.q_wage_unit*b.z_wage_price) sum_wage, ")
							      ...
							      .append(" where b.i_docno=a.i_docno and b.f_itmstatus='700' ")								  .append(team_condition)
							      .append(" and b.i_vendor='").append(iVendor).append("' ")
							      .append(condition)
								.append(itmType_restrict)
								 .append(sqlB.toString())
							...
							sql.append(" select p_cut,sum(b.z_cut_pv) sum_cut_pv ")
							      .append(" from lan:serv_infpayment b , lan:serv_infdochd a ")
							      .append(" where b.i_docno=a.i_docno and b.f_itmstatus='700' and b.i_ven_cut<>'999999' ")								  .append(team_condition)
							      .append(" and b.i_vendor='").append(iVendor).append("' ")
							      .append(condition)
								  .append(itmType_restrict)
								  .append(sqlB.toString())
					  ...
					  <tr>
					    <td width="2%" class="dotline" align="center">
							    <%
							        if (permission) {
							            %>
							            <input type="checkbox" name="i_vendor" value="<%=iVendor%>" onclick="checkAll(this,'main_check','i_vendor');">
							            <input type="hidden" name="<%=iVendor%>_name" value="<%=vendorName%>">
							            <%
							        } else {
							            %>&nbsp;<%
							        }
					    ...
					    <td width="30%" class="dotline" align="left"><a href="SERV_INFZone_Conf.jsp?itmType=<%=itmType%>&i_vendor=<%=iVendor%>"><%=vendorName+"&nbsp; "+markupPay%></a></td>
					    <td width="5%" class="dotline" align="right"><%=countDoc%></td>
...
<%
		...
		System.out.println("ERROR SERV_INFZone_List.jsp : " + e.getMessage());
