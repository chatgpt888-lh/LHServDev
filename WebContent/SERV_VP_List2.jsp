...
String jName = "SERV_VP_List2.jsp";
...
<!--
  ...
  function searchDocHD() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_VP_List2.jsp";
  ...
  function approve_job() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_VPConfServlet";
     document.forms[0].submit();
  }  
...
<input type="hidden" name="success_page" value="SERV_VP_List.jsp?d_payment=<%=dPayment%>&i_company=<%=iCompany%>">
<input type="hidden" name="error_page" value="SERV_VP_List.jsp?error=1&d_payment=<%=dPayment%>&i_company=<%=iCompany%>">
         ...
         <%   
			...
			if(user.getUserWho().equals("A")){ //lee admin
			   sqlB.delete(0,sqlB.length());
		        ...
		        /*sql.append(" select distinct d.n_project,d.i_company,d.i_project from lan:serv_payment b ")
		              .append(" left join lan:acxprojt d on d.i_company=substr(b.i_docno,1,2) and d.i_project=substr(b.i_docno,4,3) ")
		              .append(" ,lan:serv_dochd a where b.i_docno=a.i_docno and a.f_status='OPN' and b.f_itmstatus='800' ")
		              .append(condition)
		              .append(" order by d.i_company,d.i_project ");*/
		              
		         
		        sql.append("  select distinct a.i_company, a.i_project from lan:serv_payment b, lan:serv_dochd a  ")
		              .append(" where a.i_company = '"+iCompany+"' ") //
					...
					String proejectName = "";
                      ...
                      while (rs.next()) {
				            ...
				            // proejectName = "";//doString.checkString(doString.DisplayThai(rs.getString("n_project")),"");	
				             
				             sql.delete(0, sql.length());
							 sql.append("  select n_project from lan:acxprojt ")
								.append("  where 1=1 and i_company = '").append(doString.checkString(rs.getString("i_company"),"")).append("'")
								 .append(" and i_project = '").append(iProject).append("'");
								rs1 = stmt1.executeQuery(sql.toString());    				   
							    if(rs1.next()){
							       proejectName  = doString.DisplayThai(doString.checkString(rs1.getString("n_project"),""));
							    } 		            
							...
							//----====================== Count Job Form ==================----//
							sql.delete(0,sql.length());
							sql.append(" select count(distinct b.i_docno) as cnt from lan:serv_payment b,lan:serv_dochd a where b.f_itmstatus='800' ")
							     .append(" and a.i_company ='").append(iCompany).append("' ")
								 .append(" and a.i_project ='").append(iProject).append("' ")  // เพิ่มเติม by pay
							     .append(" and a.i_docno=b.i_docno ").append(condition)
							...
							sql.delete(0,sql.length());
							sql.append(" select sum(b.q_wage_unit*b.z_wage_price) sum_wage, ")
							      ...
							      .append(" from lan:serv_payment b , lan:serv_dochd a ")
					        ...
					        %>
							  <tr>
							    ...
							    <td width="30%" class="dotline" align="left"><a href="#" onclick="javascript:doSubmitForm('SERV_VP_List3.jsp?sel_project=<%=iCompany+":"+iProject%>&d_payment=<%=dPayment%>');" ><%=iCompany+"-"+iProject%> <%=proejectName%></a></td>
...
<br style="font-size:10pt">
        ...
        <table border="0" width="100%" cellspacing="0" cellpadding="0" height="30">
          <tr>
            ...
            <td class="act_tab4"><a href="<%=Constants.APP_PATH%>/SERV_VP_List.jsp?sel_project=<%=selProj%>&d_payment=<%=dPayment%>" target="_self"><img border="0" src="images/bu_back.gif" align="absmiddle" width="50" height="15"></a>&nbsp;
...
<%     System.out.println("----- SERV_VP_List2.jsp -----");
	} catch (Exception e) {
		System.out.println("ERROR SERV_VP_List2.jsp : " + e.getMessage());
