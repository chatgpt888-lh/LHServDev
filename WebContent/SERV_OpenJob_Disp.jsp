...
public boolean IsNotEditDocument(Connection conn,String docNo){
        StringBuffer sql = new StringBuffer();
        Statement stmt = null;
        ResultSet rs = null;
        ...
        try {
			...
			sql.append(" select count(*) cnt ")
				...
				//System.out.println("SQL IsNotEditDocument  :"+sql.toString());
				rs = stmt.executeQuery(sql.toString());    				   
			    if(rs.next()){
			       cnt  = rs.getInt("cnt");
			    } 		
			    if(cnt>0){ // is Not Edit
			       isRecord = true;
			    }else{ // Edit enable
			       isRecord = false;
			    }	  
            ...
            System.out.println(" IsNotEditDocument Error : " + e.getMessage());
        } finally{
            try  {
                if(rs != null) {
                    rs.close();
   ...
   public String[] GetCommentServApproved(Connection conn,String docNo){
       ...
       // String i_doc_type = "";
        ...
        try {
            ...
            stmt = conn.createStatement();
  			sql.delete(0, sql.length());
			sql.append(" Select i_remark,i_comment1,i_doc_type ")
				.append(" From lan:serv_approve ")
			    ...
			    if(rs.next()){
			       ...
			       tempStr[2] =doString.checkString(rs.getString("i_doc_type"),"");
  ...
  //---=======================================================----//   
   ...
   Hashtable docdt = new Hashtable();
	...
	try {
       ...
       isEditIcon = IsNotEditDocument(conn,iDocNo);  
      // System.out.println("---isEditIcon :"+isEditIcon);
       boolean isTurnKey = false;
       if(iDocNo.length()>0){
          String temp[] = iDocNo.split("\\-");
          isTurnKey = IsProjectTurnKey(conn,temp[0],temp[1]);
       }
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          <td width="50%" class="bigh"><img border="0" src="images/i_home.gif" align="absmiddle" width="20" height="20">&nbsp;
            Open Job</td>
          <td width="50%" align="right"><a href="/LHServ/SERV_InfJob_Disp.jsp?load=no&i_docno=<%=iDocNo%>"><img border="0" src="images/icon_view_IFJ.gif" align="absmiddle" width="120" height="34"></a></td>
        </tr>
      </table>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td class="item ; dotline01" height="22" width="13%">ประเภทใบแจ้งซ่อม :</td>
    <td height="22" width="39%" class="dotline01">
     <%=getWarrantyDesc(conn,InformTypeDDL) %>
    </td>
    <td height="22" class="item ; dotline01" width="14%">&nbsp; </td>
    <td height="22" width="34%" class="dotline01">&nbsp;</td>
  </tr>
...
<%
		...
		//out.println("docPdfPath2="+docPdfPath);
%>
<br style="font-size:10pt">
...
<br style="font-size:10pt">
			            <table border="0" width="100%" cellspacing="0" cellpadding="0">
			              <tr>
			                ...
			                <td class="item_tab3"></td>
			                <td class="textgray">&nbsp;&nbsp;<a href="http://132.146.1.105:8080/share.cgi?ssid=0myU1EL" target="blank_">ตรวจสอบรูปภาพ Backup</a> 
											&nbsp;
											<%
...
<%-- ===================================  Comment of Request ================================================ --%>
	  ...
	  <tr>
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="0">
				    <tr>
						    ...
						    <td height="22" width="76%" class="dotline01"><%=tempComment[0]%>
						    <input name="i_remark" type="hidden" value="<%//=i_remark%>" ></td>
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
	  <tr>
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr>
						    <td class="item ; dotline01" height="22" width="12%">สถานะ: &nbsp;</td>
						    <td height="22" width="76%" class="dotline01"><%
						    if("2".equals(tempComment[2])){
						    	out.println("รออนุมัติ");
						    }else if("3".equals(tempComment[2])){
						    	out.println("อนุมัติแล้ว");
						    }else if("5".equals(tempComment[2])){
						       out.println("กลับมาแก้ไขใหม่");
						    }else{
						       out.println("ไม่มีสถานะ");
						    }
						    %></td>
						    <td height="22" width="12%" class="dotline01">&nbsp;</td>
					</tr>
				    <tr>
						    <td class="item ; dotline01" height="22" width="12%">รายละเอียด : &nbsp;</td>
						    <td height="22" width="76%" class="dotline01"><%=tempComment[1]%></td>
						    <td height="22" width="12%" class="dotline01">&nbsp;</td>
					</tr>
		</table>
	</td>
...
<%
   ...
   // System.out.println("TEST : "+user.getUserWho());
   // System.out.println("edit : "+edit);
	if (!user.getUserWho().equals("J")) {
			  if (!edit.equalsIgnoreCase("NO")) {	
			     if(isEditIcon){ //true Edit i-con disable
			     	out.println("&nbsp;");
			     }else{		
				     %>
...
i_can_desc-->
<%-- ========================= Click Cancel =========================================---%>   
<div id="canBox" style="display:block ; visibility:hidden ; z-index:1 ; position:relative ; left:50% ; top:-720px ; margin-left:-250px ; margin-top:100px ; 
...
<%     System.out.println("--- SERV_OpenJob_Disp.jsp ---" );
	} catch (Exception e) {
