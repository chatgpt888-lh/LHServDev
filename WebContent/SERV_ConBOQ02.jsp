...
String jName = "SERV_ConBOQ02.jsp";
...
ข้อมูลราคา BOQ คอนโด จากส่วนกลาง</TITLE>
...
function saveData(){
    ...
    if (document.forms[0].n_desc.value=="") {
 	...
 	document.forms[0].action="<%=Constants.APP_PATH%>/SERV_ConBOQServlet";
...
function refreshPage(resetObj) {
   ...
   document.forms[0].action="<%=Constants.APP_PATH%>/SERV_ConBOQ02.jsp";
   document.forms[0].submit();
}
...
<br style="font-size:10pt">
            ...
            <table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td class="item_tab2" width="250">ข้อมูลราคา BOQ คอนโด จากส่วนกลาง</td>
   ...
   if(mode.equalsIgnoreCase("ADD")){
      ...
      sql.delete(0,sql.length());
   	  sql.append(" select max(i_seq) max_iseq from lan:serv_boq where i_seq is not null  ")
   	        .append(" and i_group='").append(iGroup).append("' and i_type='").append(iType).append("' and i_seq[1,1]='"+flag+"' ");
	  servlog.startLog(sql.toString());
      rs = stmt.executeQuery(sql.toString());
	  servlog.endLog();
		...
		if (flag.equals("C")) {
			...
			if (iSeq.trim().length()>0) {
				 seq = 0;
					...
					if (iSeq.toUpperCase().indexOf("N")==0) {
							iSeq = iSeq.substring(1);
							seq = Integer.parseInt(iSeq.trim().length()<=0 ? "0" : iSeq)+1;	
							iSeq = Integer.toString(seq);
							while (iSeq.length()<3) {
			...
			if (iSeq.toUpperCase().indexOf("E")==0) {
							iSeq = iSeq.substring(1);
							seq = Integer.parseInt(iSeq.trim().length()<=0 ? "0" : iSeq)+1;	
							iSeq = Integer.toString(seq);
							while (iSeq.length()<3) {
...
<%
		...
		System.out.println("ERROR SERV_ConBOQ02.jsp : " + e.getMessage());
