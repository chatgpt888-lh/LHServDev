...
function setDateX(strdate)
{
	year = parseInt(strdate.substring(0,4))+543;
	//with (self.opener.document.frmConfReten) {
	...
	var valuedate = "";
	if (openerForm!=null) {
	    valuedate = strdate.substring(8)+"/"+strdate.substring(5,7)+"/"+year;
		<%=dateType%>.value = valuedate;				
		<%=dateType%>.focus();
	}
	top.window.close();
}
...
function setDate(strdate){
	//alert(strdate);
	var year = parseInt(strdate.substring(0,4))+543;
	//alert("111111");
	//set Form main call opener
	var openerForm = self.opener.document.forms[0];
	//with (self.opener.document.<%//=frmParent%>) {
	with (self.opener.document.forms[0]) {
		<%=dateType%>.value = strdate.substring(8)+"/"+strdate.substring(5,7)+"/"+year;
		<%=dateType%>.focus();
	}
	//alert("22222");
...
<BODY>
<FORM NAME="frmCalendar" METHOD=POST ACTION="calendar.jsp">
...
<TABLE BORDER='0' CELLPADDING='1' CELLSPACING='2'>
  ...
  <TR><TD align="left" COLSPAN='7'><font style="color:rgb(0,0,255);">เดือน :
...
%> 
  ...
  <TR>
    <TD WIDTH=14%  style="background-color:rgb(220,240,255)"><FONT face="MS Sans Serif" color=rgb(0,50,255) size=1>Sun</FONT></TD>
    <TD WIDTH=14% style="background-color:rgb(220,240,255)"><FONT face="MS Sans Serif" color=rgb(0,50,255) size=1>Mon</FONT></TD>
    <TD WIDTH=14% style="background-color:rgb(220,240,255)"><FONT face="MS Sans Serif" color=rgb(0,50,255) size=1>Tue</FONT></TD>
    <TD WIDTH=14% style="background-color:rgb(220,240,255)"><FONT face="MS Sans Serif" color=rgb(0,50,255) size=1>Wed</FONT></TD>
    <TD WIDTH=14% style="background-color:rgb(220,240,255)"><FONT face="MS Sans Serif" color=rgb(0,50,255) size=1>Thu</FONT></TD>
    <TD WIDTH=14% style="background-color:rgb(220,240,255)"><FONT face="MS Sans Serif" color=rgb(0,50,255) size=1>Fri</FONT></TD>
    <TD WIDTH=14% style="background-color:rgb(220,240,255)"><FONT face="MS Sans Serif" color=rgb(0,50,255) size=1>Sat</FONT></TD>
  </TR>
...
<%  // Set the current day of the month
    ...
    for (int day=1; day <= daysInMonth; day++) {
      ...
      if (day == currentDay) {
		if (holiday) {             // style="background-color:rgb(220,240,255)"
...
<%
	...
	System.out.println("!!!ERROR calendar.jsp : " + e.getMessage());
