...
//           condition += " and a.i_company||':'||a.i_project='"+selProj+"'  ";
	...
	//---=========================================================================----//   
		...
		if (!jobStatus.equals("100")) {
			sql.append(" select count(*) as cnt from serv_infdochd a ")
				  .append(" where a.f_status in ('OPN','CLS') and a.i_doc_type = 'J' and (a.i_docno in ( ")
				  .append(" select  i_docno from serv_infpayment e where f_itmstatus<>'CAN' ")
				  .append(subcondition)
...
<!--
  ...
  function searchDocHD() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_INFReprint_Pay_List.jsp";
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          <td width="50%" class="bigh"><img border="0" src="images/i_home.gif" align="absmiddle" width="20" height="20">&nbsp;
            พิมพ์ใบสั่งซ่อมแก้ไขโดยผู้รับเหมา</td>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td width="100%" class="frmL">
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          <td width="14%" class="col_name">เลขที่ใบสั่งซ่อม</td>
          <td width="16%" class="col_name">วันเวลาที่แจ้ง</td>
          <td width="45%" class="col_name">ชื่อผู้รับผิดชอบ</td>
                      ...
                      if (rs.next()) {
                         if (i>=startRow && i<=endRow) {
						    ...
						    if (status.equalsIgnoreCase("INFORM")) {
						       ...
						       jspLink = "SERV_INFOpenJob_Pay_Disp.jsp?i_docno="+iDocNo;
							   status = "";
							   tmpStat = "";

						       sql.delete(0,sql.length());
						       sql.append(" select distinct f_itmstatus from serv_infpayment where f_itmstatus<>'CAN' ")
...
<%
		...
		System.out.println("ERROR SERV_INFReprint_Pay_List.jsp : " + e.getMessage());
