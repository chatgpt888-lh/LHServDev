...
<%
	...
	String jName = "SERV_PrintRetenReceipt.jsp";
...
<script language="javascript">
	...
	function searchData(act) {
		...
		document.forms[0].action="<%=request.getContextPath()%>/SERV_PrintRetenReceipt.jsp";
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          <td width="50%" class="bigh"><img border="0" src="images/i_home.gif" align="absmiddle" width="20" height="20">&nbsp;
           พิมพ์ใบเสร็จใบวางเงินค้ำประกัน</td>
    ...
    <td width="100%" class="frmL">
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
		   ...
		   <td width="12%" class="col_name" align="center"><nobr>เลขที่ป้ายต่อเติม</nobr></td>
		   ...
		   <td width="27%" class="col_name" align="center">ผู้วางเงิน</td>
	 	...
	 	<%
		    ...
		    sql.append(" select a.i_receipt, c.d_receipt, sum(a.z_amount) as sum_amt, c.i_user , c.d_receipt>='2016-09-01' as old_recpt ")
		       ...
		       .append(" and a.s_item=r.s_item and r.i_due='O5' ") // for reten pay only
			...
			while (rs.next()) {
			    ...
			    sql.append(" select h.* from lan:serv_payin p ")
			       .append(" left join lan:serv_rethd h on h.i_docno=p.i_docno and h.i_company=p.i_company and h.i_project=p.i_project and h.i_lor=p.i_lor ")
			       .append(" where p.i_company='"+iCompany+"' and p.i_project='"+iProject+"' ") 
			       .append(" and p.i_lor='"+iLor+"' and p.i_receipt='"+iReceipt+"' ");
			    rs1 = stmt1.executeQuery(sql.toString());
			    if (rs1.next()) {
			    	iSignBoard = doString.checkString(rs1.getString("i_signboard"),"");
			    	iRefCust = doString.checkString(rs1.getString("i_ret_custo"),"");
			    	nReten = doString.checkString(rs1.getString("i_reten"),"");
			    	dAcConf = doString.checkString(rs1.getString("d_ac_conf"),"");
			    	iPvNo = doString.checkString(rs1.getString("i_pvno"),"");
			    }
			    rs1.close();
			    
			    if (nReten.length()>0) {
			    	if (iRefCust.equals("1")) {
						sql.delete(0,sql.length());
						sql.append(" select * from lan:acxcusto where i_customer='"+nReten+"' ");		
						rs1 = stmt1.executeQuery(sql.toString());
						while (rs1.next()) {
							nReten  = doString.DisplayThai(doString.checkString(rs1.getString("n_prename"),""));				
							nReten += doString.DisplayThai(doString.checkString(rs1.getString("n_ncustomer"),""));				
							nReten += " "+doString.DisplayThai(doString.checkString(rs1.getString("n_scustomer"),""));	
						}
						rs1.close();				    	
			    	} else {
						sql.delete(0,sql.length());
						sql.append(" select * from lan:serv_venprj ")
						   .append(" where i_company='"+iCompany+"' and i_project='"+iProject+"' ")
						   .append(" and i_vendor='"+nReten+"' ");
						rs1 = stmt1.executeQuery(sql.toString());
						if (rs1.next()) {
							nReten  = doString.DisplayThai(doString.checkString(rs1.getString("n_pname"),""));				
							nReten += doString.DisplayThai(doString.checkString(rs1.getString("n_name"),""));				
							nReten += " "+doString.DisplayThai(doString.checkString(rs1.getString("n_sname"),""));	
						}
						rs1.close();				    	
			    	}
			    }
				...
				%>		
				  <tr>
				    ...
				    <td class="dotline" height="22" align="center">&nbsp;<%=doString.checkString(iSignBoard,"-") %></td>
				    <td class="dotline" height="22" align="center">&nbsp;<%=doString.checkString(dReceipt,"-") %></td>
				    <td class="dotline" height="22" align="center">
				    &nbsp;
				    <%
				    	if (dAcConf.trim().length()>=10 || iPvNo.trim().length()>=10) {
				    		out.println(iReceipt);
				    	} else {
				    ...
				    <td class="dotline" height="22" align="left">&nbsp;<%=doString.checkString(nReten,"-") %></td>				    
...
<%
		...
		System.out.println("ERROR SERV_PrintRetenReceipt.jsp : " + e.getMessage());
