...
<!--
  ...
  function searchDocHD() {
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_CONZone_List.jsp";
     ...
     if (show!=null) {
		...
		for (var loop=0;loop<checkbox.length;loop++) {
				...
				if (obj.checked) {
			          ...
			          html += ' สำหรับผู้รับเหมา : '+vendorName+' </td></tr></table>';
			          html += '<table border="0" width="100%" cellspacing="0" cellpadding="0">';
  ...
  function approve_job() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_ConZoneConfServlet";
     document.forms[0].submit();
  }  
...
<input type="hidden" name="success_page" value="SERV_CONZone_List.jsp?itmType=<%=itmType%>&d_payment=<%=dPayment%>">
<input type="hidden" name="error_page" value="SERV_CONZone_List.jsp?error=1&itmType=<%=itmType%>&d_payment=<%=dPayment%>">
		     ...
		     //----================== Select Data from SERV_DOCHD ================----//   
		        ...
		        rs = stmt.executeQuery(sql.toString());
					  ...
					  <tr>
					    ...
					    <td width="30%" class="dotline" align="left"><a href="SERV_CONZone_Conf.jsp?itmType=<%=itmType%>&i_vendor=<%=iVendor%>"><%=vendorName+"&nbsp; "+markupPay%></a></td>
...
<%
		...
		System.out.println("ERROR SERV_CONZone_List.jsp : " + e.getMessage());
