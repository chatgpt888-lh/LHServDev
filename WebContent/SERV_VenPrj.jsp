...
ServLog servlog = new ServLog(sessionId, userId, jName);
	...
	try {
        ...
        sql.append(" select count(*) cnt from lan:serv_venprj a where i_type<>'05' and i_type<>'06' and i_type<>'07' ").append(condition);                  
...
<!--
  ...
  function changePage(page) {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_VenPrj.jsp";
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <select name='i_type' class='box' style='width:180px' onchange="changePage(1);">
       ...
       <option value="01" <%=vendType.equals("01") ? " selected " : ""%>>01 ผู้รับเหมาซ่อม</option>
       <option value="02" <%=vendType.equals("02") ? " selected " : ""%>>02 ผู้รับเหมาสร้างเพื่อตัดเงิน</option>
    </select>
     </td>
  </tr>
</table>
    ...
    <td width="100%" class="frmL">
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td class="col_name" width="20%">ประเภทผู้รับเหมา</td>
          <td class="col_name" width="14%">รหัสผู้รับเหมา</td>
          <td class="col_name" width="40%">ชื่อผู้รับเหมา</td>
          <td class="col_name" width="10%"><nobr>&nbsp;% ดำเนินการซ่อมบ้าน&nbsp;</nobr></td>
          <td class="col_name" width="10%"><nobr>&nbsp;% ดำเนินการซ่อมสาธารณะ&nbsp;</nobr></td>
          <td class="col_name" width="10%"><nobr>&nbsp;% ดำเนินการซ่อมสาธารณู&nbsp;</nobr></td>
        </tr>
        ...
        <%
		     ...
		     //----================== Select Data from SERV_VENPRJ ================----//   
		        ...
		        sql.append(" select b.bus_name,a.* from lan:serv_venprj a ")
		              ...
		              .append(" where i_type<>'03' and i_type<>'04' and i_type<>'05' and i_type<>'06' and i_type<>'07' and i_type<>'08' ").append(condition)
                      .append( " order by   i_vendor ");
				servlog.startLog(sql.toString());
		        rs = stmt.executeQuery(sql.toString());
				servlog.endLog();
		        for (int i=0;i<maxRow;i++) { 
                      if (rs.next()) {
                         if (i>=startRow && i<=endRow) {
							...
							String pInfPay = "";
							String pPubPay = "";
							if (iType.equalsIgnoreCase("01")) {
								double percent = rs.getDouble("p_add_pay");
								pAddPay = doString.displayNumber("#,##0.00",percent);
								
								percent = rs.getDouble("p_pub_pay");
								pPubPay = doString.displayNumber("#,##0.00",percent);
								
								percent = rs.getDouble("p_inf_pay");
								pInfPay = doString.displayNumber("#,##0.00",percent);
							}
					        %>
					        <tr>
					          ...
					          <td class="dotline" align="left" width="20%"><%=iType.equalsIgnoreCase("01") ? "ผู้รับเหมาซ่อม" : "ผู้รับเหมาสร้าง"%></td>
					          <td class="dotline" align="center" width="14%"><%=iVendor%></td>
					          <td align="left" class="dotline" width="40%"><%=vendorName%></td>
					          <td align="right" class="dotline" width="10%"><%=pAddPay%>&nbsp;</td>
							  <td align="right" class="dotline" width="10%"><%=pPubPay%>&nbsp;</td>
					          <td align="right" class="dotline" width="10%"><%=pInfPay%>&nbsp;</td>
					        </tr>					        			        
					        <%
 					         line++;                         
                         } // end if check row
	           ...
	           while (line<displayLine) {
	               line++;
			        ...
			        <tr>
			          ...
			          <td align="left" class="dotline" width="40%">&nbsp;</td>
...
<%
		...
		System.out.println("ERROR SERV_VenPrj.jsp : " + e.getMessage());
