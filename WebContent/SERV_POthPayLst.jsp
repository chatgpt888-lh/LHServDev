...
%><HTML><HEAD><TITLE>รายการเบิกงวดงานบ้าน และงานสาธารณะ อื่นๆ</TITLE>
...
function deleteData() {
    document.forms[0].mode.value = "D";
 	document.forms[0].action="<%=Constants.APP_PATH%>/SERV_POthPayServlet";
...
function chckAll(){
   ...
   function queryProject() {
       document.forms[0].action = "SERV_POthPayLst.jsp?search=y";
       document.forms[0].submit();
   }
...
<BODY leftMargin=0 topMargin=0 marginheight="0" marginwidth="0"><FORM NAME="frmOthPayLst" METHOD=POST ACTION="/LHServ/SERV_POthPayLst.jsp"><input type="hidden" name="mode" value="D">
...
<br style="font-size:10pt">
            ...
            <table border="0" width="100%" cellspacing="0" cellpadding="0">              <tr>                <td class="item_tab1"><img border="0" src="images/i_i.gif" align="absmiddle" width="20" height="20"></td>                <td class="item_tab2" width="250">สรุปรายการเบิก</td>                <td class="item_tab3"></td>                <td>&nbsp;</td>               
              </tr>
            </table>
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>          <td class="col_name" width="5%"><input type="checkbox" name="selAll" onClick="javascript:chckAll();"></td>          <td class="col_name" width="15%">ประจำเดือน</td>          <td class="col_name" width="10%">ค่าแรงงาน</td>
          ...
          <td class="col_name" width="30%">สถานะ</td>
           		...
           		int line = 0;		        rs = stmt.executeQuery("SELECT i_month, z_wage, z_control, d_payment, f_tran FROM lan:serv_pothpayment WHERE i_company = '"+comId+"' AND i_project = '"+projId+"' AND i_vendor = '"+vendor+"' ORDER BY i_month");                while (rs.next() == true) {
...
<%if (!status.equals("T")) {%>
						<a href="SERV_POthPay.jsp?comId=<%=comId%>&projId=<%=projId%>&venId=<%=vendor%>&payMonth=<%=payMonth%>&payYear=<%=payYear%>"><%=DateUtil.ifxToThaiDateNoTime(rs.getString("I_MONTH"))%></a>
...
<br style="font-size:10pt">
        ...
        <table border="0" width="100%" cellspacing="0" cellpadding="0" height="30">
          <tr>
            ...
            <a href="SERV_POthPay.jsp?comId=<%=comId%>&projId=<%=projId%>&venId=<%=vendor%>"><img border="0" src="images/act_add.gif"                                   
...
<%	} catch (Exception e) {		System.out.println("ERROR SERV_POthPayLst.jsp : " + e.getMessage());		throw new ServletException(e.getMessage());	} finally {		// Clean up.		try {			if (rs != null) rs.close();			if (stmt != null) stmt.close();			if (conn != null) conn.close();		}		catch( SQLException ignore ){}	}%>