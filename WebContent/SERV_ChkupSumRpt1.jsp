...
<%
   ...
   //----============ Declare Variables for input data ===========----//
	...
	try {
		...
		sql.append("select max(i_year) as max_y , max(i_month) as max_m  from serv_chkmain ");
...
<!--
  ...
  function goReport() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_ChkupSumRpt1_1.jsp";
...
<FORM ACTION="SERV_ChkupSumRpt1_1.jsp" METHOD="POST" name="frm">
...
<br style="font-size:10pt">
          ...
          <td class="item_tab2" width="200">กรุณาเลือกช่วงเวลาและประเภทที่ต้องการ</td>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr> 
				...
				<%=common.genMonthListbox("month_report","01"," class='box' ")%>
				</td>
                <td width="3%" height="22" class="item ; dotline01">ปี :</td>
                <td width="80%" height="22" class="dotline01"> 
				<%=common.genYearListbox("year_report",cur_year," class='box' ",2007,(Integer.parseInt(cur_year)-2007))%>
				 </td>
              </tr>
              <tr> 
                <td class="item ; dotline01" height="22">ประเภท :</td>
                <td height="22" colspan="3" class="dotline01">
				<input name="report_type" type="radio" value="3">
                  3 เดือน &nbsp;&nbsp; <input type="radio" name="report_type" value="6">
                  6 เดือน&nbsp;&nbsp; <input type="radio" name="report_type" value="12">
                  12 เดือน &nbsp;&nbsp; </td>
              </tr>
			  </table>
                ...
                <td width="38%" valign="top" bgcolor="#F5F5F5">
				  <select size="15" name="all_proj" multiple class="box" style="width:300px" ondblclick="MoveSelect(frm.all_proj, frm.sel_proj,'SEL');">
					<%
						...
						rs = stmt.executeQuery("SELECT proj_id FROM lan:serv_pstaff WHERE user_id = '"+user.getUserID()+"' AND com_id = 'LH' AND proj_id = 'ALL'");
						if (rs != null) {
							if (rs.next() == true) {
								allProj = true;
							}
...
<%
		...
		System.out.println("ERROR SERV_ChkupSumRpt1.jsp : " + e.getMessage());
