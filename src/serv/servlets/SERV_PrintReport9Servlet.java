 ...
 * @author
 */
public class SERV_PrintReport9Servlet extends DBServlet  {
	...
	*/
	...
	public void genHeaderPage(PdfPTable table,String headerReport,String currDate,Font microssfont,Font microssfont_HD,Font microssfont_MINI) throws Exception {
			...
			cell.setBorder(0);
			table.addCell(cell);			
			cell = new PdfPCell(new Phrase("แบบรายงาน : SERV_PrintReport9 ,  "+currDate, microssfont));
			cell.setHorizontalAlignment(Rectangle.ALIGN_RIGHT);
	...
	public void genVendorData(PdfWriter writer,Connection conn,String iVendor,String headerReport,boolean moreVendor,String markupPay,Document document) throws Exception {
		...
		try {
			...
			//----============================= Get Data ================================----//
			sql.delete(0,sql.length());
			sql.append(" select a.i_docno,a.i_lock,c.n_project,a.i_type_cutlck ")
				  .append(" ,d.n_desc,e.i_house from lan:serv_payment b,lan:serv_dochd a ")
			...
			while (rs.next()) {
				...
				String cutType = doString.checkString(rs.getString("n_desc"),"");
				...
				//--=========================== Table Header =============================---//	
				String tableDesc = " เลขที่แปลง "+iLock+" บ้านเลขที่ "+iHouse+"  , เลขที่ใบแจ้งซ่อม "+iDocNo+"  "+cutType;
				...
				if (iVendor.length()>0) { sql.append(" and b.i_vendor='").append(iVendor).append("' "); }
				if (selProj.length()>0 && !selProj.equals("ALL")) { 
					//sql.append(" and substr(b.i_docno,1,2)||':'||substr(b.i_docno,4,3)='").append(selProj).append("' ");
			...
			System.out.println("SERV_PrintRerort9Servlet Error : "+e.getMessage()); 
		} finally {
			if (rs!=null) rs.close();  
...
*/
		   ...
		   if (selProj.trim().length()>2) {
			...
			conn = null;
			 ...
			 de.printStackTrace();
			 System.err.println("error SERV_PrintRerort9Servlet  DOCUMENT: " + de.getMessage());
		} catch (Exception e) {
