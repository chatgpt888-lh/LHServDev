...
String jName = "SERV_Report9_2.jsp";
...
*/
	...
	//---=========================================================================----//
        ...
        sql.append(" select distinct i_vendor from serv_dochd a,serv_payment b ")
		  ...
		  .append(" and b.i_ven_cut = '"+cut_vendor+"' ");
	...
	//out.println(sql.toString());
		...
		servlog.endLog();
        while (rs.next()) {
            vendorList.addElement(doString.checkString(doString.DisplayThai(rs.getString("i_vendor")),""));  
        }
        rs.close();
	//---=========================================================================----//
...
<!--
  ...
  //   document.forms[0].action="<%=Constants.APP_PATH%>/SERV_Report9_2.jsp";
    ...
    <td width="100%" class="frmLR" align="center"> <table border="0" width="100%" cellspacing="0" cellpadding="0">
	  ...
	  <%=common.genProjectListboxByUserId(user.getUserID(),"sel_project",selProj," onchange='changePage(1);' class='box' style='width:250px' ",false)%>
        ...
        <tr>
          ...
          <td width="57%" height="22" class="dotline01">
	  <%=common.genVendorList("i_vendor",selProj,vendor," class='box' style='width:250px' ")%>
	    &nbsp;&nbsp;
	   </td>
        </tr>
        <tr>
  ...
  int line = 0;
	...
	String cutType = "";
		 ...
		 for (int v=0;v<vendorList.size();v++) {
			...
			sql.append(condition);
		        sql.append(" group by a.i_docno,a.i_lock,c.n_project,a.i_type_cutlck , d.n_desc,e.i_house ")
					            ...
					            <table border="0" width="100%" cellspacing="0" cellpadding="0">
					              <tr>
					                ...
					                <td class="textgray"><%=nProject%>&nbsp;&nbsp;
					                  เลขที่ใบแจ้งซ่อม :
					                  <a href="SERV_OpenJob_Disp.jsp?i_docno=<%=docNo%>&popup=y&edit=no" target="_blank"><%=docNo%></a>&nbsp;
...
<%
		...
		System.out.println("ERROR SERV_Report9_2.jsp : " + e.getMessage());
