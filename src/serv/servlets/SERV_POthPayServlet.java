 ...
 * @author
 */
public class SERV_POthPayServlet extends DBServlet  {
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		String successPage = "SERV_POthPayLst.jsp?sel_project="+selProj+"&vendor="+vendor;
		...
		SERV_CommonData common = null;
		 try {
			...
			if (mode.equalsIgnoreCase("S")) {
				...
				sql.append("update lan:serv_pothpayment set z_wage = ")
					.append(wageAmnt)
					.append(", z_control = ")
					.append(cntrlAmnt)
					.append(", d_payment = '")
				...
				if (rowEffected <= 0) {
					sql.delete(0,sql.length());
					sql.append("insert into lan:serv_pothpayment (i_company,i_project,i_vendor,i_month,z_wage,z_control,d_payment,f_tran")
							  .append(" ) values ( ")
					...
					rowEffected = stmt.executeUpdate(sql.toString());
					if (rowEffected != 1) {
						throw new Exception("SERV_POTHPAYMENT: Wrong insert count");
					}
				}
	 			...
	 			if (delMnth!=null) {
	 			 	 for (int i=0;i<delMnth.length;i++) {
	 			 		 	...
	 			 		 	if (!mnthDate.equals("")) {
	 							sql.delete(0,sql.length());
	 							sql.append("delete from lan:serv_pothpayment ")
