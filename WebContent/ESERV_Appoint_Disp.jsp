...
<%!
	...
	private static String getNameSys(ArrayList sysList,String code){
	 String sysName = "";
	 if(sysList!=null && sysList.size()>0){  
	    HashMap hashMap = null;
		for (Iterator iter = sysList.iterator(); iter.hasNext(); ) {
			hashMap = (HashMap)iter.next();
			if(code.equals(hashMap.get("xCODE").toString())){
			   sysName = hashMap.get("xNAME").toString();
			}
		}
	   }
	  return sysName;
 ...
 * Last modify :2013.10.15  modify subject i_type off appion date
 * version :1.0
...
function doADD(){
	 ...
	 document.forms[0].action="<%=request.getContextPath()%>/ESERV_AppointDateServlet?cmd=formLoad&mode=add&projectNameThai="+proj;
...
function doDelete(DateDel){
   ...
   if(confirm("คุณต้องการลบรายการกำหนดเวลาเข้าตรวจสอบใช่หรือไม่?")==true){
	   ...
	   document.forms[0].action="<%=request.getContextPath()%>/ESERV_AppointDateServlet?cmd=delete&projectNameThai="+proj;
	   document.forms[0].submit();
	}
}

function doSave(){
	 document.forms[0].action="<%=request.getContextPath()%>/ESERV_AppointDateServlet?cmd=save";
	 document.forms[0].submit();
}
-->
</script>
...
<input type="hidden" name="sysTypeDDL" value="<%=codeDDL%>"> 
...
<br style="font-size:10pt">
            <table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td class="item_tab2" width="250">กำหนดเวลานัดเข้าตรวจสอบรายการซ่อม</td>
  ...
  <tr>
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
	  <tr>
	    ...
	    <td height="22" width="38%" class="dotline01"><%=projectNameThai%></td>	
	    <td class="item ; dotline01" height="22" width="12%" valign="top">ระบบ :</td>
	    <td height="22" width=""38%"" class="dotline01"><%=getNameSys(sysList,codeDDL)%></td>							
	    </tr>
	</table>
</td>
</tr>
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0">     
        ...
        if(dayList!=null && dayList.size()>0){
          ...
          for(int i=0;i<dayList.size();i++){//Loop date Unique
            		...
            		%>
            		<tr>
					<td class="dotline" width="10%" align="center">
						<a href="javascript:doDelete('<%=dayList.get(i)%>');"   >
						 <img alt="Delete" border="0" src="images/i_delete.gif" ></a>
					</td>
...
<p align="left">
 ...
 HashMap hashMap = null;
	...
	for (Iterator iter = sysList.iterator(); iter.hasNext(); ) {
	  ...
	  %>
	   <%=tagDel %><font color="ff7537"><%=code %> : <%=name %></font><%=cTagDel %></br>
	  <%
...
<p align="left">
<font color="#ff7537">
ESV  : ระบบ e-Service<br></font>
<font color="#7bd148">SVC : ระบบ Service Center</font></p>
-->
