	...
	if(document.forms[0].fromDate.value ==""){
	 ...
	 if(isAll){//เลือกโครงการเท่ากับ ทุกโครงการ
		 if(document.forms[0].projSelDDL.options.length>=1){
			 	...
			 	//do_totals1() ;
				//document.forms[0].action="<%//=request.getContextPath()%>/SERV_ReportSvcServlet?cmd=GenReport";
...
function doSubmitForm(){
	if(validDate(document.forms[0].fromDate,document.forms[0].toDate)){
	    do_totals1();
		document.forms[0].action="<%=request.getContextPath()%>/SERV_ReportSvcServlet?cmd=GenReport";
		document.forms[0].submit();
	}
...
function validDate(fdate,tdate) {
	   ...
	   // alert(edate+","+emonth+","+eyear);
	    ...
	    if (sdate.length==0 && smonth.length==0 && syear.length==0 &&
	        ...
	        document.forms[0].fromDAppoint.focus();
	        document.forms[0].fromDAppoint.select();
	        return false;
	       // return true;
	    }
		...
		/*var dateDiff = daysBetween(new Date(parseInt(syear,10),parseInt(smonth,10)-1,parseInt(sdate,10)),new Date(parseInt(eyear,10),parseInt(emonth,10)-1,parseInt(edate,10)));	 
		//alert(dateDiff);	
		//document.forms[0].MaxDate.value = dateDiff;
		if(dateDiff>93 ){
			alert("กรุณาตรวจสอบวันที่ด้วย (เลือกช่วงระยะเวลาภายใน 3 เดือนเท่านั้น!!).");
			document.forms[0].fromDate.focus();
			document.forms[0].fromDate.select();
			return false;
		}*/
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
  	...
  	<tr>
    <td class="item ; dotline01" height="22" width="15%">
				<font style="color:rgb(255,100,0)">เลือกช่วงวันนัดหมาย :</font> </td>
    <td height="22" width="%" colspan="3" class="dotline01">
  				
				&nbsp;
				<input type="text" name="fromDate" value="" size="15" class="boxC" style="width:80px" readonly="readonly" ><A HREF="javascript:selChqDate('fromDate');"><IMG border="0" src="images/i_calendar.gif" align="absmiddle" width="18" height="18"></A>
				
				<font style="color:rgb(255,100,0)">&nbsp;&nbsp;&nbsp;&nbsp; ถึง :</font>
				<input type="text" name="toDate" value="" size="15" class="boxC" style="width:80px" readonly="readonly" ><A HREF="javascript:selChqDate('toDate');"><IMG border="0" src="images/i_calendar.gif" align="absmiddle" width="18" height="18"></A> &nbsp;&nbsp;&nbsp;
	    </td>
  	</tr>
