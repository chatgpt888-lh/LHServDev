...
String jName = "SERV_FCAMReport.jsp";
...
doString str = new doString();
   ...
   String subcondition = "";
	...
	try {
		...
		String option = "";	
		String position = "ALL", owner = "ALL", i_company = "", i_project = "", n_project = "";
		if (request.getParameter("position") != null) {
				position = doString.DisplayThai(doString.checkString(request.getParameter("position")));
		} // End if	
		if (request.getParameter("owner") != null) {
				owner = doString.DisplayThai(doString.checkString(request.getParameter("owner")));
		} // End if	
...
*/
		...
		String orderby = doString.checkString(request.getParameter("order"), "a.i_header, a.i_cardno");
	...
	if (d_start.length()>0) {
	   condition += " and a.i_date = '"+d_start+"' ";
	}
...
function submit()  {		
		Finger.action = "/LHServ/SERV_FCAMReport.jsp";
		Finger.submit();
}
...
<FORM NAME="Finger" METHOD="POST" ACTION="SERV_FCAMReport.jsp">
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
 <!-- <tr>
   ...
   <tr>
    ...
    <select name='position' class='box' style='width:250px'  >
    <OPTION value="ALL">--------------------- ทุกตำแหน่ง ---------------------</OPTION>
<%	
	         sql.delete(0, sql.length());
			 sql.append("SELECT * from lan:drive_owner ")
				  .append("where i_type = '02' ")
				  .append("order by i_type, i_code ");
			rs = stmt.executeQuery(sql.toString());
			while (rs.next()==true) {
	
			option = "";
			if (position.equals(doString.DisplayThai(doString.checkString(rs.getString("i_code"))))) {
				option = " Selected ";
			} // End if
%>
			<OPTION value="<%=doString.DisplayThai(doString.checkString(rs.getString("i_code")))%>" <%=option%>>
			<%=doString.DisplayThai(doString.checkString(rs.getString("n_desc")))%>
			</OPTION>
<%			
		} // End while
%></SELECT>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;สังกัดบริษัท : <select name='owner' class='box' style='width:250px'  >
...
<%	
			...
			option = "";
			if (owner.equals(doString.DisplayThai(doString.checkString(rs.getString("i_code"))))) {
				option = " Selected ";
			} // End if
%>
			<OPTION value="<%=doString.DisplayThai(doString.checkString(rs.getString("i_code")))%>" <%=option%>>
...
//out.println("subcondition =="+subcondition);
		     ...
		     //----================== Select Data from SERV_DOCHD ================----//   
				...
				String n_owner = "";
				/*select b.*, a.*
					from lan:serv_tstaff b left join lan:serv_finger a
					on a.i_date = '20140825'  
					and a.i_cardno = b.i_cardno 
					where b.i_header = 'Admin Dept' 	*/


			sql.delete(0,sql.length());	
			sql.append("select * from lan:serv_tstaff ")
				 .append("where i_header = 'Admin Dept' ");
	if (!position.equals("ALL")) {
		    sql.append("and n_job = '"+position+"' ");
	}
	if (!owner.equals("ALL")) {
		    sql.append("and i_dept = '"+owner+"' ");
	}
			sql.append("order by i_cardno ");
			...
			while (rs1.next()) {
					...
					sql.delete(0,sql.length());	
					sql.append("select a.i_cardno, a.i_name, a.i_header, a.i_date, a.i_checkin, a.i_checkout, b.n_job, b.z_wage, b.i_dept ")
						 .append("from lan:serv_finger a left join lan:serv_tstaff b on a.i_cardno = b.i_cardno and a.i_header = 'Admin Dept' "+condition)
						 .append("where a.i_cardno = b.i_cardno ")	
						 .append("and a.i_cardno = '"+doString.checkString(rs1.getString("i_cardno"))+"' ")
			...
			/*		 if (!h_proj.equals(doString.checkString(rs.getString("i_header")))) {					 
					...
					if (rs2.next()==true) {
							n_job = doString.checkString(doString.DisplayThai(rs2.getString("n_desc")));
					}
					...
					rs2 = stmt2.executeQuery(sql.toString());					
					if (rs2.next()==true) {
							n_owner = doString.checkString(doString.DisplayThai(rs2.getString("n_desc")));
					}

							 //doString.DisplayThai(rs.getString("n_job")

%>
					        <tr>
					          <td width="5%" align="center" class="dotline"><%=line%></td>
					          <td width="15%" class="dotline ; item" align="center"><A HREF="SERV_FCAMDetail.jsp?cardno=<%=doString.checkString(rs.getString("i_cardno"))%>"><%=doString.checkString(rs.getString("i_cardno"),"-")%></A></td>
...
%>
							...
							<tr>
					          <td width="5%" align="center" class="dotline"><%=line%></td>
					          <td width="15%" class="dotline ; item" align="center"><%=doString.checkString(rs1.getString("i_cardno")) %></td>
					          <td width="20%" class="dotline" align="left"><%=doString.checkString(doString.DisplayThai(rs1.getString("i_name"))) %></td>
...
<!--
							<tr>
					          ...
					          <td width="15%" align="right" class="dotline ; item">Total&nbsp;</td>
							   <td width="15%" align="right" class="dotline ; item"><%=sum_wage%>&nbsp;</td>
					        </tr>
							-->
<%
...
<%
		...
		System.out.println("ERROR SERV_FCAMReport.jsp : " + e.getMessage());
