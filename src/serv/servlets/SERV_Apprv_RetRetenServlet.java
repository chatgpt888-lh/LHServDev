 ...
 * @author
 */
public class SERV_Apprv_RetRetenServlet extends DBServlet  {
...
/*
		...
		String errorPage = "SERV_Apprv_RetReten.jsp?error=1&refresh=yes&i_docno="+iDocNo;
		String otherMsg = "";
		String errorCode = "";
		   ...
		   sql.delete(0,sql.length());
				   ...
				   //-----=============================================================-----//				   
					...
					sql.delete(0,sql.length());
				 	sql.append(" update lan:serv_rethd set ")
					      .append(" i_doc_status='").append(approveFlag).append("' , ")
			       ...
			       //-----================== Insert new Flow for this document ===============-----//
					...
					sql.append(" insert into lan:serv_apprv (i_flow,i_docno,i_doc_status,i_apprv,d_apprv,c_apprv) ")
						  ...
						  .append(" '").append(approveFlag).append("' , ")
						  .append(" '").append(oldApprv).append("' , ") // old is iCurApprv
						  .append(" today , '").append(cApprv).append("') ");      		
					stmt.executeUpdate(sql.toString());
					//-----=============================================================-----//
					...
					//String header = "<HTML><HEAD><TITLE></TITLE><META http-equiv=\"Content-Type\" content=\"text/html; charset=TIS-620\"><META http-equiv=\"Content-Language\" content=\"th\"></HEAD><BODY BGCOLOR=\"#FFFFFF\"><FONT size=\"2\" face=\"Microsoft Sans Serif\">";  /* 2009/08/27 */
					String header = "<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=windows-874\">";
					header += "<META http-equiv=\"Content-Language\" content=\"th\"></HEAD><BODY BGCOLOR=\"#FFFFFF\"><FONT size=\"2\" face=\"Microsoft Sans Serif\">";
					String footer = "</FONT></BODY></HTML>";

					//---============ Send E-Mail to Account if this document is approve ================----//
					if (approveFlag.trim().equalsIgnoreCase("V")) {
						//----- Read Approver Details for send mail --------//						
						sql.delete(0,sql.length());
						sql.append("select b.i_employ,b.user_email from docflow:icv_acpr a ")
						      .append(" left join lan:useracl b on b.i_employ=a.i_employ and b.user_acl='S' ")
						      .append(" where a.i_ap_typ='GRP' and a.i_com_emp='LH' ")
							  .append(" and i_company='").append(iCompany)
						...
						if (rs.next()) {
							...
							if (email.trim().length()>0) {
								...
								//subject = doString.UnicodeToMS874(subject); /* 2009/08/27 */
						
								String url = Constants.APP_SERVER+Constants.APP_PATH+"/SERV_Conf_ARecevChq.jsp?i_employ="+iEmploy;
								url += "&i_company="+iCompany+"&i_project="+iProject+"&sel_type=E";
								...
								//mailText = doString.UnicodeToMS874(mailText);	 /* 2009/08/27 */			
		
								LHMail mail = new LHMail();
								////////mail.sendMailHtml(Constants.LH_HOST,Constants.LH_DOMAIN,Constants.BOQ_SENDER,email,"",subject,header+mailText+footer); /* 2009/08/27 */
								//mail.sendMailHtml(Constants.LH_HOST,Constants.LH_DOMAIN,Constants.BOQ_SENDER,email,"",doString.UnicodeToMS874(subject),doString.MS874ToUnicode(header+mailText+footer));
								//mail.sendBBMail(Constants.LH_HOST,Constants.LH_DOMAIN,Constants.BOQ_SENDER,email,"",doString.MS874ToUnicode(subject),doString.MS874ToUnicode(header+mailText+footer));
					...
					//---====== Send E-Mail to i_reten_payback for tell this document is approve or route back =====----//					
					   //------ Validate Email before send ------//
					   if (retenEmail.trim().length()>0) {
						   String subject = " เอกสารขอคืนเงินค้ำประกัน เลขที่ "+iDocNo+" สถานะ "+statusDesc;
						   //subject = doString.UnicodeToMS874(subject); // 2019-05-27
								
						   String mailText = " เอกสารขอคืนเงินค้ำประกัน เลขที่ "+iDocNo+" ของ "+retenEmp+" สถานะ "+statusDesc;
						   if (approveFlag.trim().equalsIgnoreCase("V")) mailText += "<br> กรุณาส่งเอกสารให้ทางบัญชีภายใน 3 วัน";
