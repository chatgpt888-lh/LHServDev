...
<%
   ...
   String accountId = doString.checkString(request.getParameter("account"),"");   
    // --------------------------  Create by ....pay 10/05/2007 ----------------------------- //
...
*/
	...
	//---=========================================================================----//
        ...
        sql.append(" select distinct i_vendor from serv_infdochd a,serv_infpayment b ")
	      .append(" where  a.f_status in ('OPN','CLS') and a.i_docno = b.i_docno and b.f_itmstatus = 'CLS' ");
	if (cut_vendor.length()>0) { sql.append(" and b.i_ven_cut = '"+cut_vendor+"' "); }      // = cut_vendor
	if (accountId.length()>0) { sql.append(" and b.i_account='").append(accountId).append("' "); }      // = accountId
		  sql.append(" and b.i_itmtype = '"+itmType+"' ");
	if (vendor.length()>0) { sql.append(" and b.i_vendor='").append(vendor).append("' "); }      // = iVendor
...
<!--
  ...
  function searchDocHD() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_INFReport9_2.jsp";
...
function printReport() {
   ...
   //document.forms[0].action='http://www9.lh.co.th/LHServ/SERV_PrintReport9Servlet<%=iDocNo.length()>0 ? "?i_docno="+iDocNo : ""%>';
    ...
    <td width="100%" class="frmLR" align="center"> <table border="0" width="100%" cellspacing="0" cellpadding="0">
	  ...
	  <%=common.genProjectListboxByUserId(user.getUserID(),"sel_project",selProj," onchange='changePage(1);' class='box' style='width:250px' ",false)%>
        ...
        <tr>
          ...
          <td width="57%" height="22" class="dotline01">
		  <%=common.genItmTypeOpenJobDropDown("itmType",itmType," onchange='changePage(1);' class='box' style='width:250px' " ) %>
	    &nbsp;&nbsp;
	   </td>
        </tr>
        <tr>
          ...
          <td width="57%" height="22" class="dotline01">
	  <%=common.genVendorList("vendor",selProj,vendor," class='box' style='width:250px' ")%>
	    &nbsp;&nbsp;
	   </td>
        </tr>
        <tr>
		 ...
		 for (int v=0;v<vendorList.size();v++) {
		     ...
		     //----================== Select Data from SERV_INFDOCHD ================----//
		        ...
		        sql.append(" select first ").append(endRow).append(" a.i_docno,c.n_project ")
		              ...
		              .append(" where ")
		              .append("  a.f_status in ('OPN','CLS') and a.i_docno = b.i_docno ")
						...
						<%
							...
							while (rs1.next()) {
							    ...
							    if ((!iVenCut.equals("999999")) || (iVenCut.equals("999999") && pCut>0)) {
							        ...
							        remark += doString.DisplayThai(rs1.getString("bus_name"));
							...
							if (sumCutOnly>0) {
							   %>
							        <tr>
							          <td class="solidline ; item"  align="left"> รวมตัดเงินผู้รับเหมางทั้งหมด <%=format.format(sumCutOnly)%> บาท</td>
								          <td class="dotline" align="right" width="3%" valign="top">&nbsp;</td>
								          <td class="dotline" align="right" width="7%" valign="top">&nbsp;</td>
								          <td class="dotline" align="right" width="7%" valign="top">&nbsp;</td>
								          <td class="dotline" align="right" width="3%" valign="top">&nbsp;</td>
...
-->
							 ...
							 %>
							        <tr>
							          <td class="solidline ; item" colspan="2" align="center">รวมทั้งหมด</td>
							          <td class="solidline ; item" align="right" width="7%">&nbsp;</td>
...
<br style="font-size:10pt">
	   ...
	   <!--  <img border="0" src="images/act_print.gif" onclick="printReport();"                                   
            ...
            <td class="act_tab4"><a href="<%=Constants.APP_PATH%>/SERV_INFReport8.jsp?sel_project=<%=selProj%>&i_vendor=<%=iVendor%>" target="_self"><img border="0" src="images/bu_back.gif" align="absmiddle" width="50" height="15"></a>&nbsp;
...
<%
		...
		System.out.println("ERROR SERV_INFReport9_2.jsp : " + e.getMessage());
