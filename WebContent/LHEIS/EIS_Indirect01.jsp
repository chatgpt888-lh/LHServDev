...
try {
	...
	String year = Integer.toString(rightNow.get(Calendar.YEAR)+543);   //+543
...
%>
<FORM NAME="frmEIS" METHOD=GET ACTION="EIS_Indirect01.jsp">
...
<%
		...
		//out.println("year=="+year);
			...
			if (typecost.equals("A")) {
				...
				sum_all = "sum(z_expst_m1) m1, sum(z_expst_m2) m2, sum(z_expst_m3) m3, sum(z_expst_m4) m4, sum(z_expst_m5) m5, ";
				sum_all += "sum(z_expst_m6) m6, sum(z_expst_m7) m7, sum(z_expst_m8) m8, sum(z_expst_m9) m9, sum(z_expst_m10) m10, ";
				sum_all += "sum(z_expst_m11) m11, sum(z_expst_m12) m12, sum(z_expst_m13) m13, ";
				sum_all += "sum(z_expst_m1+z_expst_m2+z_expst_m3+z_expst_m4+z_expst_m5+z_expst_m6) sm1, sum(z_expst_m7+z_expst_m8+z_expst_m9+z_expst_m10+z_expst_m11+z_expst_m12) sm2";
				...
				sum_all = "sum(z_expal_m1) m1, sum(z_expal_m2) m2, sum(z_expal_m3) m3, sum(z_expal_m4) m4, sum(z_expal_m5) m5, ";
				sum_all += "sum(z_expal_m6) m6, sum(z_expal_m7) m7, sum(z_expal_m8) m8, sum(z_expal_m9) m9, sum(z_expal_m10) m10, ";
				sum_all += "sum(z_expal_m11) m11, sum(z_expal_m12) m12, sum(z_expal_m13) m13, ";
				sum_all += "sum(z_expal_m1+z_expal_m2+z_expal_m3+z_expal_m4+z_expal_m5+z_expal_m6) sm1, sum(z_expal_m7+z_expal_m8+z_expal_m9+z_expal_m10+z_expal_m11+z_expal_m12) sm2";
				...
				sum_all = "sum(z_expst_m1+z_expal_m1) m1, sum(z_expst_m2+z_expal_m2) m2, sum(z_expst_m3+z_expal_m3) m3, sum(z_expst_m4+z_expal_m4) m4, sum(z_expst_m5+z_expal_m5) m5, ";
				sum_all += "sum(z_expst_m6+z_expal_m6) m6, sum(z_expst_m7+z_expal_m7) m7, sum(z_expst_m8+z_expal_m8) m8, sum(z_expst_m9+z_expal_m9) m9, sum(z_expst_m10+z_expal_m10) m10, ";
				sum_all += "sum(z_expst_m11+z_expal_m11) m11, sum(z_expst_m12+z_expal_m12) m12, sum(z_expst_m13+z_expal_m13) m13, ";
				sum_all += "sum(z_expst_m1+z_expal_m1+z_expst_m2+z_expal_m2+z_expst_m3+z_expal_m3+z_expst_m4+z_expal_m4+z_expst_m5+z_expal_m5+z_expst_m6+z_expal_m6) sm1, ";
				sum_all += "sum(z_expst_m7+z_expal_m7+z_expst_m8+z_expal_m8+z_expst_m9+z_expal_m9+z_expst_m10+z_expal_m10+z_expst_m11+z_expal_m11+z_expst_m12+z_expal_m12) sm2";
...
// ------------------------------------ Open Project ---------------------------------------
		...
		while (rs.next()) {
					...
					if (type.equals("V1")) {								
							  //------------------ ค่าใช้จ่าย ---------------------
							   sql.delete(0, sql.length());
							   sql.append("select "+sum_detail+", i_company, i_project from lan:acbexpsm ")					
											...
											r1_sm2 = rs1.getDouble("sm2")/1000; 
													  ...
													  if (z_net_ytd == 0) {
														...
														if (rs3.next()) {
															...
															n_proj = doString.DisplayThai(doString.checkString(rs3.getString("n_project")));
														}
%>
...
<%							//  --------------   TOTAL  V1 -----------------										
			 ...
			 if (type.equals("V2")) {  
							 ...
							 //------------------ ต้นทุนทางอ้อม ---------------------
						   sql.delete(0, sql.length());
						   sql.append("select "+sum_detail+", i_company, i_project from lan:acbidrsm ")					
								.append("where i_year = '"+year+"' ")
								.append("and i_type "+type_id+" ")
								.append("and i_company = '"+doString.checkString(rs.getString("i_company"))+"' ")
								.append("and i_project = '"+doString.checkString(rs.getString("i_project"))+"' ")
...
if (type.equals("V1") || type.equals("V3")) {		   
				   ...
				   sql.delete(0, sql.length());
				   sql.append("select "+sum_all+" from lan:acbexpsm a ")
						.append("where a.i_year = '"+year+"' ")
...
<% 
		...
		if (type.equals("V2") || type.equals("V3")) {	
					   ...
					   sql.delete(0, sql.length());
					   sql.append("select "+sum_all+" from lan:acbidrsm a ")
							.append("where a.i_year = '"+year+"' ")
							.append("and a.i_type "+type_id+" ")
							.append("and a.i_acctno = 'TOTAL' ")
							.append("and a.i_project != '099' ")
...
<%
	...
	System.out.println("!!!ERROR EIS_Indirect01.jsp : " + e.getMessage());
	System.out.println("!!!ERROR EIS_Indirect01.jsp SQL : " + sql.toString());
