 ...
 *
 */
 public class InitChkLckServlet extends DBServlet {	 
	 private static String cName = "/LHServ/InitChkLckServlet";
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if (targetPage.equals("")) {
			targetPage = "/LHServ/SERV_ChkLckLst.jsp?Project="+project+"&chkMonth="+chkMonth+"&chkYear="+chkYear;
		} else {
...
*/
					rsChkup = ustmt.executeQuery("SELECT i_sort FROM lan:acscontr WHERE i_company = '"+comId+"' AND i_project = '"+projId+"' AND f_contr IS NULL AND d_close_law >= '"+begDate+"' AND d_close_law <= '"+endDate+"' UNION SELECT i_lock FROM lan:serv_flood WHERE i_company = '"+comId+"' AND i_project = '"+projId+"' AND i_chkseq = "+Integer.toString(seqNo));
					if (rsChkup != null) {
						while (rsChkup.next() == true) {
							lockId = doString.checkString(rsChkup.getString("I_SORT"));
							System.out.println(lockId);							
							deny = false;
							rs = stmt.executeQuery("SELECT i_lock FROM lan:serv_denchkup WHERE i_company = '"+comId+"' AND i_project = '"+projId+"' AND i_lock = '"+lockId+"' AND i_chkseq = "+Integer.toString(seqNo));
							if (rs != null) {
								if (rs.next() == true) {
									deny = true;
								}
	...
	*/									
								...
								if (!chckup && !closeDate.equals("")) {							
									...
									//rs = stmt.executeQuery("SELECT i_docno, f_status, i_month FROM lan:serv_chkuplck WHERE i_company = '"+comId+"' AND i_project = '"+projId+"' AND i_lock = '"+lockId+"' AND i_chkseq = "+Integer.toString(seqNo) + " AND f_status != 'D'");								
									if (rs != null) {
										if (rs.next() == true) {
											chckup = true;
...
*/				    	
	        System.out.println("ERROR /LHServ/InitChkLckServlet : " + e.getMessage());
