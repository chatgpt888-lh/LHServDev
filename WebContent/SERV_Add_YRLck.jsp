...
String jName = "SERV_Add_YRLck.jsp";
...
function CalcAmnt() {
var amount = eval(frmEdtInfra.amount.value);
var intRate = eval(frmEdtInfra.interest.value)/100.00;
var interest = amount * intRate;
amount += interest;
frmEdtInfra.payAmnt.value = commaSplit(formatCurrency(amount));
}

function IntKeyPress(keycode) {
	if (keycode == 13) { //Enter
		CalcAmnt();
	} else {
...
<FORM name="frmEdtInfra" method="post" action="SERV_Add_YRLck.jsp">
...
%>                      
  ...
  <tr>
                  <td height="22" class="item ; dotline01" width="8%">ปี : 
                    :</td>
...
<%
	...
	sortId = beg_lock;
	sql.delete(0, sql.length());
	sql.append("SELECT i_lor FROM lan:serv_inflck WHERE i_company = '"+comId+"' AND i_project = '"+projId+"' AND i_sort = '"+beg_lock+"'");
	servlog.startLog(sql.toString());
	rs = stmt.executeQuery(sql.toString());
	servlog.endLog();
	if (rs != null) {
	...
	if (rsTime != null) {
		while (rsTime.next() == true) {
			...
			betweenDate = doString.DisplayThai(Period.getBetween(startDate, endDate));
			sql.delete(0, sql.length());
			...
			if (rsContr != null) {
				if (rsContr.next() == true) {
					match = false;
/*
					rs = stmt.executeQuery("SELECT i_lor FROM lan:serv_infhd WHERE i_company = '"+comId+"' AND i_project = '"+projId+"' AND i_sort = '"+sortId+"' AND d_end >= '"+startDate+"'");
					if (rs != null) {
						if (rs.next() == true) {
							match = true;
						}
...
%>
                <tr> 
                  ...
                  <td height="22" width="28%" class="dotline01"><%=betweenDate%>
					<INPUT type="hidden" name="chkTime" value="<%=startDate%><%=endDate%>">
				  </td>
                  <td height="22" width="12%" class="item ; dotline01">จำนวนเงิน :</td>
                  <td height="22" width="48%" class="dotline01">
					<INPUT type="hidden" name="amount" value="<%=doString.displayNumber("#########.00", amount)%>">
				  <%=doString.displayNumber("###,###,###.00", amount)%>&nbsp; บาท
				  </td>
                </tr>
                <tr> 
                  <td height="22" class="item ; dotline01" width="12%">ดอกเบี้ย :</td>
                  <td height="22" width="28%" class="dotline01"><INPUT type="text" name="interest" class="box" value="0" style="width:60px" onKeyPress="if(IntKeyPress(event.keyCode)==false){event.returnValue = false;}" onBlur="CalcAmnt()"> %</td>
                  <td height="22" width="12%" class="item ; dotline01">เป็นเงิน :</td>
                  <td height="22" width="48%" class="dotline01">
					<INPUT type="text" name="payAmnt" class="boxD" value="<%=doString.displayNumber("###,###,###.00", amount)%>" style="width:60px">บาท
				  </td>
                </tr>
<%        	                
					} else {
%>
                <tr> 
                  ...
                  <td height="22" width="12%" class="item ; dotline01">จำนวนเงิน :</td>
                  <td height="22" width="48%" class="dotline01">ตั้งยอดแล้ว</td>
                </tr>
<%
					}
				} else {// end if
%>
                <tr> 
                  ...
                  <td height="22" width="28%" class="dotline01"><%=betweenDate%></td>
                  <td height="22" width="12%" class="item ; dotline01">จำนวนเงิน :</td>
                  <td height="22" width="48%" class="dotline01">จ่ายแล้ว</td>
                </tr>
<%
				}
				rsContr.close();
...
<%
	...
	System.out.println("ERROR SERV_Add_YRLck.jsp : " + e.getMessage());
