 ...
 */
public class SERV_PrintAccRetenServlet extends DBServlet  {
...
/*
		...
		String funcType = doString.checkString(req.getParameter("func_type"),"C");
...
//			String approveName = "";
			...
			//System.out.println("this sql not execute : "+sql.toString());
...
//		 	      .append(" trim(f.n_prename_th)||trim(f.n_nemploy_th)||' '||trim(f.n_semploy_th) as approve_name , ")
				  .append(" e.i_tel , s.n_desc as n_paybnk , a.* from lan:serv_rethd a ")
...
//			      .append(" left join docflow:acemploy f on f.i_employ=a.i_cur_apprv ")
			...
			if (funcType.equalsIgnoreCase("C")) {				  
				sql.append(" and a.i_doc_status='V' ");
			} else if (funcType.equalsIgnoreCase("A")) {
				sql.append(" and a.i_doc_status='A' ");				
			} 
			//--- 2023-02-22 , for case reprint after end PV or later ---//
			else {
				sql.append(" and a.i_doc_status='"+funcType+"' ");
			}
			//-----------------------------------------------------------//
			rs = stmt.executeQuery(sql.toString());
			while (rs.next()) {
...
//				     iCurrApprv = doString.checkString(rs.getString("i_cur_apprv"),"");
			...
			sql.append(" select trim(nvl(n_pname,''))||trim(nvl(n_name,''))||' '||trim(nvl(n_sname,'')) as cust_name,i_tel ")
			...
			//-----========== If company is not LH , get Company Name  ============-----//
			String companyName = "";
			if (!iCompany.equalsIgnoreCase("LH")) {
				sql.delete(0,sql.length());
				sql.append("select * from lan:acxcompa where i_company='").append(iCompany).append("' ");
				rs = stmt.executeQuery(sql.toString());
				while (rs.next()) {
			...
			sql.delete(0,sql.length());
			sql.append(" select * from lan:serv_apprv where i_flow='R' and i_doc_status in ('O','V') ")
				  .append(" and i_docno='").append(iDocNo).append("' ");		  
			rs = stmt.executeQuery(sql.toString());
			while (rs.next()) {
				String docStatus = doString.checkString(rs.getString("i_doc_status"),"");
				if (docStatus.equalsIgnoreCase("O")) {
					iApprv1 = doString.checkString(rs.getString("i_apprv"),"");
				} else if (docStatus.equalsIgnoreCase("V")) {
					iApprv2 = doString.checkString(rs.getString("i_apprv"),"");
				}
			} // end while rs
			rs.close();
			...
			//----- Read Bank name , branch and account , 2011-02-28 --------//	
			//------- 2018-06-05 , check account seperate by province -------//	
			String regionCond = "";
			sql.delete(0,sql.length());
			sql.append(" select * from lan:pay_prjprov ")
			   .append(" where i_company='"+iCompany+"' and i_project='"+iProject+"' ");
			rs = stmt.executeQuery(sql.toString());
			if (rs.next()) {
				//--- region ---//
				regionCond = " and a.i_province='"+doString.checkString(rs.getString("i_prov"),"")+"' ";
			} else {
				//--- bangkok ---//
				regionCond = " and a.user_dept='99' ";
			}
			rs.close();			
			
			String acctNo = "";	
			sql.delete(0,sql.length());
			sql.append(" select b.i_acct_no,a.* from docflow:icv_acctn a,lan:acraccnt b ")
			      .append(" where a.i_com_exp='").append(iCompany).append("' ")
			      .append(" and a.i_system='RET' and b.i_bank=a.i_pv_bank ")
			      .append(" and b.i_branch=a.i_pv_bran and b.i_account=a.i_pv_acctyp  ")
			      .append(regionCond); // 2018-06-05
			rs = stmt.executeQuery(sql.toString());
			if (rs.next()) {
				acctNo = doString.checkString(rs.getString("i_acct_no"),"");
				acctNo += " ("+doString.checkString(rs.getString("i_pv_bank"),"");
				acctNo += "-"+doString.checkString(rs.getString("i_pv_bran"),"");
				acctNo += "-"+doString.checkString(rs.getString("i_pv_acctyp"),"")+") ";
			} 
			rs.close();
			...
			String apprv1Details[] = getEmployeeDetails(stmt,iApprv1);
			String apprv2Details[] = getEmployeeDetails(stmt,iApprv2);
			...
			Font microssfont_MED_BOLD = new Font(bfb, 13, Font.BOLD); // 2022-06-30
			...
			//----========= Add Space for Header ===========----//
					...
					if (iCompany.equalsIgnoreCase("LH")) {		
						...
						cell.setBorder(0);
						table.addCell(cell);		
						cell = new PdfPCell(new Phrase("ฉบับบัญชี  " , microssfont));
					...
					//cell = new PdfPCell(new Phrase("\nใบขอคืนเงินค้ำประกันการปลูกสร้างอาคารหรือต่อเติม\n\n", microssfont_HD));
					cell = new PdfPCell(new Phrase("\nใบขอคืนเงินค้ำประกันความเสียหายสาธารณูปโภคและพื้นที่บริการสาธารณะ\n\n", microssfont_HD));
					cell.setHorizontalAlignment(Rectangle.ALIGN_CENTER);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(100);
					cell.setBorder(0);
					...
					cell = new PdfPCell(new Phrase("ชื่อผู้รับเรื่อง : "+doString.MS874ToUnicode(retenDetails[1]), microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(leftColumnWidth);
					cell.setBorder(0);
					...
					cell = new PdfPCell(new Phrase("จ่ายเช็ค : "+acctNo, microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_TOP);
					cell.setColspan(100);
					cell.setBorder(0);
					...
					cell.setBorder(4);
					table.addCell(cell);
					cell = new PdfPCell(new Phrase("\n\n "+doString.MS874ToUnicode(retCustName), microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(25);
					cell.setBorder(2);
					table.addCell(cell);							
					cell = new PdfPCell(new Phrase("\n\n ผู้ขอคืนเงินประกัน", microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(15);
					cell.setBorder(0);
					...
					cell.setBorder(0);
					table.addCell(cell);
					cell = new PdfPCell(new Phrase("\n\n "+doString.MS874ToUnicode(retenDetails[1]), microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(25);
					cell.setBorder(2);
					table.addCell(cell);							
					cell = new PdfPCell(new Phrase("\n\n ผู้รับเรื่องขอคืนเงินประกัน", microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(21);
					cell.setBorder(8);
					...
					cell.setBorder(4);
					table.addCell(cell);						
					cell = new PdfPCell(new Phrase(" ( "+doString.MS874ToUnicode(retenDetails[2])+" ) ", microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(46);
					cell.setBorder(8);
					...
					cell.setBorder(0);
					table.addCell(cell);
					cell = new PdfPCell(new Phrase("\n\n "+doString.MS874ToUnicode(apprv1Details[1]), microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(25);
					cell.setBorder(2);
					...
					cell.setBorder(4);
					table.addCell(cell);						
					cell = new PdfPCell(new Phrase(" ( "+doString.MS874ToUnicode(apprv1Details[2])+" ) ", microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(46);
					cell.setBorder(8);
					...
					cell.setBorder(0);
					table.addCell(cell);
					cell = new PdfPCell(new Phrase("\n\n "+doString.MS874ToUnicode(apprv2Details[1]), microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(25);
					cell.setBorder(2);
					table.addCell(cell);							
					cell = new PdfPCell(new Phrase("\n\n ผู้อนุมัติ", microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(21);
					cell.setBorder(8);
					...
					cell.setBorder(4);
					table.addCell(cell);						
					cell = new PdfPCell(new Phrase(" ( "+doString.MS874ToUnicode(apprv2Details[2])+" ) ", microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(46);
					cell.setBorder(8);
					...
					//---- 2022-06-30 , print payin or payto notice -----//
					if (iPayType.equalsIgnoreCase("PAYIN")) {
		        		...
		        		//// line 1
						//cell = new PdfPCell(new Phrase("* เป็นรายการโอนเงินเข้าบัญชีธนาคาร ", microssfont));
		        		cell = new PdfPCell(new Phrase("* เป็นรายการโอนเงินเข้าบัญชี ", microssfont));
						cell.setColspan(100);
						cell.setBorder(0);
						table.addCell(cell);
						...
						cell = new PdfPCell(new Phrase(" "+doString.MS874ToUnicode(nPayBnk), microssfont_MED_BOLD));
						cell.setColspan(25);
						cell.setBorder(2);
						table.addCell(cell);							
						
						cell = new PdfPCell(new Phrase(" เลขที่บัญชี", microssfont));
						cell.setColspan(9);
						cell.setBorder(0);
						table.addCell(cell);
						
						cell = new PdfPCell(new Phrase(" "+iPayAcc, microssfont_MED_BOLD));
						cell.setColspan(15);
						cell.setBorder(2);
						table.addCell(cell);				
						
						cell = new PdfPCell(new Phrase(" ชื่อบัญชี ", microssfont));
						cell.setColspan(7);
						cell.setBorder(0);
						table.addCell(cell);
						
						cell = new PdfPCell(new Phrase(" "+doString.MS874ToUnicode(retCustName), microssfont_MED_BOLD));
						cell.setColspan(30);
						cell.setBorder(2);
						table.addCell(cell);
						...
						cell = new PdfPCell(new Phrase("* เป็นรายการออกเช็คคืนเงิน สั่งจ่ายในนาม ", microssfont));
						cell.setColspan(31);
						cell.setBorder(0);
						table.addCell(cell);
					...
					cell.setBorder(4);
					table.addCell(cell);		
					cell = new PdfPCell(new Phrase("                    ( "+doString.MS874ToUnicode(retenDetails[1]+" - "+retenDetails[2])+" )\n\n", microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(50);
					cell.setBorder(8);
					...
					cell.setBorder(4);
					table.addCell(cell);		
					cell = new PdfPCell(new Phrase("                    ( "+doString.MS874ToUnicode(apprv1Details[1]+" - "+apprv1Details[2])+" )\n", microssfont));
					cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
					cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
					cell.setColspan(50);
					cell.setBorder(8);
			...
			cell.setBorder(6);
			table.addCell(cell);		
			cell = new PdfPCell(new Phrase("                    ( "+doString.MS874ToUnicode(apprv2Details[1]+" - "+apprv2Details[2])+" )\n", microssfont));
			cell.setHorizontalAlignment(Rectangle.ALIGN_LEFT);
			cell.setVerticalAlignment(Rectangle.ALIGN_MIDDLE);
			cell.setColspan(50);
			cell.setBorder(10);
			...
			conn = null;
			 ...
			 de.printStackTrace();
			 System.err.println("error SERV_PrintAccRetenServlet  DOCUMENT: " + de.getMessage());
		} catch (Exception e) {
