 ...
 * @author
 */
public class PrintInfLetter4Servlet extends DBServlet  {
	...
	public void printLetter(Document document, String printDate, String inf_year, String projNme, String custName, String houseId,float price,String startDate,String endDate,double amount) throws Exception {
	  ...
	  table.addCell(addCellData("ฉบับที่ 5","R","C","",100,microssfont));
	  ...
	  table.addCell(addCellData("ขอความร่วมมือในการชำระค่าบริการสาธารณะ (ครั้งที่ 2)","L","C","",95-3,microssfont));
	  ...
	  table.addCell(addCellData("สิ่งที่ส่งมาด้วย","L","C","",15-2,microssfont_BOLD));
	  table.addCell(addCellData("ใบแจ้งการชำระเงิน พร้อมแบบฟอร์มการชำระเงินที่ธนาคาร (ใบ PAY-IN)","L","C","",95+2,microssfont_BOLD));
	  
	  table.addCell(addCellData(" ","L","C","",100,microssfont));
	  
	  table.addCell(addCellData(" ","L","C","",5+3,microssfont));
	  table.addCell(addCellData("ตามที่บริษัท แลนด์ แอนด์ เฮ้าส์ จำกัด (มหาชน) ได้ขอความร่วมมือจากท่านในการชำระค่าบริการสาธารณะประจำปี "+inf_year+" นั้น","L","C","",95-3,microssfont));
	  table.addCell(addCellData("ปรากฏว่าท่านยังไม่ได้ชำระค่าบริการดังกล่าว โดยท่านยังคงค้างชำระค่าบริการสาธารณะประจำปี "+inf_year,"L","C","",100,microssfont));
	  table.addCell(addCellData("ตั้งแต่เดือน "+startMnth+" พ.ศ. "+startYear+" ถึงเดือน "+endMnth+" พ.ศ. "+endYear+" เป็นจำนวนเงิน "+doString.displayNumber("###,###,###.00", amount)+" บาท ("+doString.MS874ToUnicode(currencyToThai.getString())+")","L","C","",100,microssfont));
	  
	  table.addCell(addCellData(" ","L","C","",5+3,microssfont));
	  table.addCell(addCellData("ดังนั้นบริษัทฯ จึงใคร่ขอให้ท่านได้โปรดชำระเงินค่าบริการสาธารณะตามจำนวนเงินดังกล่าวข้างต้น โดยใช้","L","C","",95-3,microssfont));
	  table.addCell(addCellData("เอกสารการชำระเงินของธนาคารที่ได้แนบมาพร้อมกับหนังสือฉบับนี้ โดยท่านต้องนำจำนวนเงินตามใบแจ้งการชำระเงิน","L","C","",100,microssfont));
	  
	  table.addCell(addCellData("ภายในกำหนด 7 วันนับตั้งแต่วันที่ท่านได้รับหนังสือฉบับนี้  หากพ้นกำหนดระยะเวลาดังกล่าวแล้ว บริษัทฯ มีความจำเป็นจะต้อง","L","C","",100,microssfont_BOLD));
	  table.addCell(addCellData("ดำเนินการตามที่เห็นสมควรต่อไป","L","C","",100,microssfont_BOLD));
	  
	  table.addCell(addCellData(" ","L","C","",100,microssfont));
	  table.addCell(addCellData(" ","L","C","",5+3,microssfont));
	  table.addCell(addCellData("จึงเรียนมาเพื่อทราบและโปรดดำเนินการภายในระยะเวลาที่กำหนดดังกล่าวนี้ด้วยและหากท่านได้ชำระเงินค่าบริการ","L","C","",95-3,microssfont));
	  table.addCell(addCellData("สาธารณะไปก่อนที่จะได้รับหนังสือฉบับนี้ บริษัทฯ ต้องกราบขออภัยมา ณ โอกาสนี้","L","C","",100,microssfont));
...
*/			
			...
			if (rs != null) {
				while (rs.next() == true) {
					...
					//------ generate data -------//
					printLetter(document,printDate,inf_year,projNme,custName,houseId,price,startDate,endDate,amount);
					if (lockNo < num_lock)
						document.newPage();
				}// end while lock
				rs.close();
		  ...
		  conn = null;
			 ...
			 System.err.println("error PrintInfLetter4Servlet  DOCUMENT: " + de.getMessage());
		} catch (Exception e) {
