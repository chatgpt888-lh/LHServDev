...
String sessionId = "0";
String userId = "";
String jName = "SERV_Conf_RetReten2.jsp";
   ...
   if (checkUser!=null) {
	   userWho = checkUser.getUserWho();
		...
		sessionId = checkUser.getsessionId();
   } else {
	...
	sql.append(" select b.i_company||b.i_project||' | '||b.n_project as project_name , c.n_desc , e.c_apprv , s.n_desc as n_paybnk , ")
	      ...
	      //-----------------------------------------//	  		      
	      .append(" where a.i_doc_status in ('G','O') and a.z_reten=a.z_recv_reten and a.i_staff_payback is not null ")
...
function approveRetReten() {
     ...
     if (confirm("คุณแน่ใจว่าต้องการ Approve ใบรายการนี้  ?")) {	
        document.forms[0].approve_flag.value="O";
...
function routebackRetReten() {
     ...
     if (confirm("คุณแน่ใจว่าต้องการ Route Back ใบรายการนี้ ?")) {	
        document.forms[0].approve_flag.value="U";
        document.forms[0].submit();
     }
}
</script>
...
<form action="<%=Constants.APP_PATH%>/SERV_Conf_RetReten2Servlet" method="post">
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td width="100%" class="frmLRpad01"><textarea rows="5" name="c_apprv" class="box" style="width:100%" cols="20" <%=iDocStatus.equals("G") ? "" : " disabled"%>><%=cApprv%></textarea></td>
  </tr>
</table>
...
<table cellspacing="0" cellpadding="0" width="100%">
				    ...
				    if (iDocStatus.equals("G")) {
							...
							sql.delete(0,sql.length());
							sql.append(" select i_apprv from lan:serv_apprv where ")
								  .append(" i_docno='").append(iDocNo).append("' ")
								  .append(" and i_doc_status='G' ");
							servlog.startLog(sql.toString());
							rs = stmt.executeQuery(sql.toString());
							servlog.endLog();
							if (rs.next()) {
								String iEmploy = doString.checkString(rs.getString("i_apprv"),"");
								String curApprv[] = getEmployeeDetails(stmt1,iEmploy,servlog);
								out.println("<font color='#000096'>"+doString.DisplayThai(curApprv[1])+"</font> &nbsp; ( "+doString.DisplayThai(curApprv[2])+" )");
							} // end while rs
							rs.close();
				    }
...
<table cellspacing="0" cellpadding="0" width="100%">
                                  <tr>
                                    ...
                                    <td width="250">ผู้อนุมัติโต๊ะสุดท้าย</td>
                                  </tr>
</table>
<!-- Approve#2 End -->
