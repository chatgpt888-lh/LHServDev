 ...
 * description :  Report  สรุปรายงานเก็บก่อนโอน
***************************************************/
--%>
<%!
...
<HTML>
<HEAD>
<TITLE>Report สรุปรายงานเก็บก่อนโอน</TITLE>
...
function doSubmit(){
   ...
   var checkRbt = getCheckedValue(obj);
     if(checkRbt==""){
		...
		document.forms[0].rbtType.select();
	    return;
	}else if(checkRbt=="A"){
	    if(document.forms[0].AmmDDL_F1.value ==""){
			document.forms[0].AmmDDL_F1.focus();
			document.forms[0].AmmDDL_F1.select;
			alert("กรุณาเลือกเดือน!!");
		    return;
		}else if(document.forms[0].AyyDDL_F1.value ==""){
			document.forms[0].AyyDDL_F1.focus();
			document.forms[0].AyyDDL_F1.select;
			alert("กรุณาเลือกปี!!");
		    return;
		}else if(document.forms[0].AmmDDL_T1.value ==""){
			document.forms[0].AmmDDL_T1.focus();
			document.forms[0].AmmDDL_T1.select;
			alert("กรุณาเลือกเดือน!!");
		    return;
		}else if(document.forms[0].AyyDDL_T1.value ==""){
			document.forms[0].AyyDDL_T1.focus();
			document.forms[0].AyyDDL_T1.select;
			alert("กรุณาเลือกปี!!");
		    return;
		}else{
			if(document.forms[0].AyyDDL_F1.value>document.forms[0].AyyDDL_T1.value){
				document.forms[0].AyyDDL_F1.focus();
				document.forms[0].AyyDDL_F1.select;
				alert("กรุณาตรวจสอบปีที่เลือกด้วย!!");
			    return;
			}else if((document.forms[0].AyyDDL_F1.value==document.forms[0].AyyDDL_T1.value) &&
			        (parseInt(document.forms[0].AmmDDL_F1.value ,10) > parseInt(document.forms[0].AmmDDL_T1.value,10)) ){
				 document.forms[0].AmmDDL_F1.focus();
				 document.forms[0].AmmDDL_F1.select;
				 alert("กรุณาตรวจสอบเดือนที่เลือกด้วย!!");
			     return;    
			}else{
				doValidate1();
			}
	    ...
	    if(document.forms[0].BmmDDL_F2.value ==""){
			document.forms[0].BmmDDL_F2.focus();
			document.forms[0].BmmDDL_F2.select;
			alert("กรุณาเลือกเดือน!!");
		    return;
		}else if(document.forms[0].ByyDDL_F2.value ==""){
			document.forms[0].ByyDDL_F2.focus();
			document.forms[0].ByyDDL_F2.select;
			alert("กรุณาเลือกปี!!");
		    return;
		}else if(document.forms[0].BmmDDL_T2.value ==""){
			document.forms[0].BmmDDL_T2.focus();
			document.forms[0].BmmDDL_T2.select;
			alert("กรุณาเลือกเดือน!!");
		    return;
		}else if(document.forms[0].ByyDDL_T2.value ==""){
			document.forms[0].ByyDDL_T2.focus();
			document.forms[0].ByyDDL_T2.select;
			alert("กรุณาเลือกปี!!");
		    return;
		}else{
			//alert(document.forms[0].BmmDDL_F2.value+","+document.forms[0].BmmDDL_T2.value);
			//alert(document.forms[0].ByyDDL_F2.value+","+document.forms[0].ByyDDL_T2.value);
			if( document.forms[0].ByyDDL_F2.value > document.forms[0].ByyDDL_T2.value){
				document.forms[0].ByyDDL_F2.focus();
				document.forms[0].ByyDDL_F2.select;
				alert("กรุณาตรวจสอบปีที่เลือกด้วย!!");
			    return;
			}else if((document.forms[0].ByyDDL_F2.value==document.forms[0].ByyDDL_T2.value ) &&
			        (parseInt(document.forms[0].BmmDDL_F2.value,10) > parseInt(document.forms[0].BmmDDL_T2.value ,10))){
				 document.forms[0].BmmDDL_F2.focus();
				 document.forms[0].BmmDDL_F2.select;
				 alert("กรุณาตรวจสอบเดือนที่เลือกด้วย!!");
			     return;    
...
function doValidate1(){
	if(document.forms[0].projSelDDL.options.length==0) {
	 ...
	 if(isAll){//เลือกโครงการเท่ากับ ทุกโครงการ
	 	...
	 	onPleaseWait() ;
		document.forms[0].action="<%=request.getContextPath()%>/SERV_RptTransAfterEndServlet?cmd=GenReport";
    ...
    <td width="100%" class="BD" >   
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          ...
          &nbsp;Report สรุปรายงานเก็บก่อนโอน</td>
        </tr>
      </table>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
    ...
    <input name="rbtType" type="radio" value="A" >เดือน/ปี ที่แจ้งซ่อม :</td>
    <td width="80%" class="dotline01">
    	<select size="1" class="box2" style="width:100px" name="AmmDDL_F1" >
     ...
     &nbsp;
     <select size="1" class="box2" style="width:100px" name="AyyDDL_F1" >
     ...
     &nbsp; ถึง &nbsp;
         	<select size="1" class="box2" style="width:100px" name="AmmDDL_T1" >
	 	 <option value=''>--เดือน --</option>
	 	<%
	 	 selected = "";
	 	curMonth++;
     ...
     &nbsp;
     <select size="1" class="box2" style="width:100px" name=AyyDDL_T1 >
        <option value=''>---ปี---</option>
        <%
        selected = "";
		for(int i = 0;i<yyList.size();i++){	
 ...
 <tr>
    <td class="item ; dotline01" height="22" width="20%">
    <input name="rbtType" type="radio" value="B" >เดือน/ปี ที่โอน :</td>
    <td width="80%" class="dotline01">

    	<select size="1" class="box2" style="width:100px" name="BmmDDL_F2" >
	 	 <option value=''>--เดือน --</option>
	 	<%
	 	 selected = "";
	 	curMonth++;
     ...
     &nbsp;
     <select size="1" class="box2" style="width:100px" name="ByyDDL_F2" >
        <option value=''>---ปี---</option>
        <%
        selected = "";
		for(int i = 0;i<yyList.size();i++){	
     ...
     &nbsp; ถึง &nbsp;
         	<select size="1" class="box2" style="width:100px" name="BmmDDL_T2" >
	 	 <option value=''>--เดือน --</option>
	 	<%
	 	 selected = "";
	 	curMonth++;
     ...
     &nbsp;
     <select size="1" class="box2" style="width:100px" name="ByyDDL_T2" >
        <option value=''>---ปี---</option>
        <%
        selected = "";
		for(int i = 0;i<yyList.size();i++){	
		  ...
		  <%
			   ...
			   if(projectDDL!=null && projectDDL.size()>0){
					...
					while(it.hasNext()){
							 ...
							 strName = 	hashMap.get("name").toString();
