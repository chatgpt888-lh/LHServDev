...
<!--
  ...
  function searchDocHD() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_INFManager_Conf.jsp";
  ...
  function approve_job() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_INFManagerConfServlet";
     document.forms[0].submit();
  }  
  ...
  function reject_job() {
      ...
      if (confirm("คุณแน่ใจว่าต้องการ Reject ใบงานที่เลือกนี้ ?")) {
		   var sub = document.getElementsByName("i_itmjob");
		    if (sub.length!=null) {
				for (var i=0;i<sub.length;i++) {
					if (sub[i].checked) {
					  var comment = document.getElementsByName(sub[i].value+"_comment");
					  if (comment!=null && trim(comment[0].value)=="") {
				  ...
				  var comment = document.getElementsByName(sub[0].value+"_comment");
				  if (comment!=null && trim(comment[0].value)=="") {
			         alert(" กรุณากรอกหมายเหตุ เกี่ยวกับการ Reject !");
			         //comment.focus();
			         return false;					     
...
<%
		     ...
		     //----================== Select Data from SERV_DOCHD ================----//   
		        ...
		        sql.append(" select first ").append(endRow).append(" a.i_docno,c.n_project ")
		              ...
		              .append(" a.f_status='OPN' "+team_condition+" and b.i_docno=a.i_docno ")
					            ...
					            <table border="0" width="100%" cellspacing="0" cellpadding="0">
					              <tr>
					                ...
					                <td class="textgray"><%=nProject%>&nbsp;&nbsp;
					                  เลขที่ใบแจ้งซ่อม :
					                  <a href="SERV_INFOpenJob_Pay_Disp.jsp?i_docno=<%=iDocNo%>&popup=y&edit=no" target="_blank"><%=iDocNo%></a></td>                
					              </tr>
					            </table>
...
<br style="font-size:10pt">
        ...
        <table border="0" width="100%" cellspacing="0" cellpadding="0" height="30">
          <tr>
            ...
            <td class="act_tab4"><a href="<%=Constants.APP_PATH%>/SERV_INFManager_List.jsp?itmType=<%=itmType%>" target="_self"><img border="0" src="images/bu_back.gif" align="absmiddle" width="50" height="15"></a>&nbsp;
...
<%
		...
		System.out.println("ERROR SERV_INFManager_Conf.jsp : " + e.getMessage());
