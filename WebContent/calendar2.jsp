...
function setDate(strdate)
{
	...
	var valuedate = strdate.substring(8)+"/"+strdate.substring(5,7)+"/"+year;
	
    try {
		openerDoc.forms[0].elements("<%=dateType%>").value = valuedate;		
		openerDoc.forms[0].elements("<%=dateType%>").focus();    	
    } catch(err) {
        openerDoc.getElementById("<%=dateType%>").value = valuedate;	
        openerDoc.getElementById("<%=dateType%>").focus(); 
    }	
	
	/*
	if (openerForm!=null) {
	    
		var txtBox = openerForm.getElementById("<%=dateType%>");
		if (txtBox!=null) {
			txtBox.value = valuedate;		
			txtBox.focus();
		}
	}
	...
	if (openerForm!=null) {
		...
		openerForm.elements("<%=dateType%>").value = valuedate;		
		
		openerForm.elements("<%=dateType%>").focus();
	}	
	*/
	top.window.close();
}
...
<BODY>
<FORM NAME="frmCalendar" METHOD=POST ACTION="calendar2.jsp">
...
<%
	...
	System.out.println("!!!ERROR calendar2.jsp : " + e.getMessage());
