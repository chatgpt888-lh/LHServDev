...
<!--
function CancelJob() {
	frmDispJob.action = "/LHServ/CancelInfJobServlet";
	frmDispJob.target="";
	frmDispJob.submit();
}
function printOpenJob() {
	frmDispJob.action = "/LHServ/SERV_PrintInfOpenJobServlet";
	frmDispJob.target="_blank";
	frmDispJob.submit();
}
function editOpenJob(){
	document.forms[0].action = "/LHServ/ServOpenJob?actionMode=editOpenJob&mode=E&i_itmtype=<%=itmType%>";
	frmDispJob.target="";
...
<FORM NAME="frmDispJob" METHOD=POST ACTION="">
...
try {
	...
	if (rs != null) {
		if (rs.next() == true) {
			...
			comment = doString.DisplayThai(doString.checkString(rs.getString("c_reject")));	
		}
		rs.close();
		rs=null;
	}
	
	rs = stmt.executeQuery("SELECT f_approve FROM lan:serv_infdocap WHERE i_docno = '"+docNo+"' AND i_chart_grp = 'S'");
	if (rs != null) {
		if (rs.next() == true) {
	...
	if (rs != null) {
		if (rs.next() == true) {
			chartGrp = doString.checkString(rs.getString("user_who"));		
		}
		rs.close();
		rs=null;
	}
...
<%if (status.equals("R")) {%>
			...
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
			  <tr>
			    ...
			    <textarea rows="5" name="Comment" class="box" style="width:100%" cols="20"><%=comment%></textarea>
			    </td>
			  </tr>
			</table>
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
...
<%
		...
		System.out.println("ERROR SERV_INFOpenJob_Disp.jsp : " + e.getMessage());
...
<%if (mode.equals("E")) {%>
            <a href="javascript:editOpenJob();"><img border="0" src="images/act_edit.gif"
    			onmouseout=nereidFade(this,70,50,5)
                  	onmouseover=nereidFade(this,100,50,5)
                  	style="cursor:hand;FILTER: alpha(opacity=70)" width="70" height="27"></A>&nbsp;
            <a href="javascript:CancelJob()"><img border="0" src="images/act_cancel.gif"
    			onmouseout=nereidFade(this,70,50,5)
                  	onmouseover=nereidFade(this,100,50,5)
                  	style="cursor:hand;FILTER: alpha(opacity=70)" width="70" height="27"></a>&nbsp;
<%}%>
<%if (!status.equals("R")) {%>
            <a href="javascript:printOpenJob()"><img border="0" src="images/act_print001.gif"
    			onmouseout=nereidFade(this,70,50,5)
                  	onmouseover=nereidFade(this,100,50,5)
                  	style="cursor:hand;FILTER: alpha(opacity=70)" width="70" height="27"></a>
<%}%>                  	
