...
public class SERV_CommonData
{

    private Connection conn;

    public SERV_CommonData(Connection conn)
    {
        this.conn = conn;
    }
    ...
    public String genAllProjectListbox(String name, String value, String params, boolean allProj)
    {
        ...
        try
        {
            ...
            html.append("<select id='").append(name).append("' name='").append(name).append("' ").append(params).append(" onchange='queryProject()'>");
    ...
    public String genProjectListboxByUserId(String userId, String name, String value, String params, boolean getAllProj)
    {
        ...
        try
        {
            ...
            {
                sql.delete(0, sql.length());
                sql.append(" select a.i_company com_id,a.i_project proj_id,b.n_project from lan:serv_venprj a ").append(" left join lan:acxprojt b on b.i_company=a.i_company and b.i_project=a.i_project").append(" where a.i_vendor='").append(iPerson).append("' ").append(" and a.i_type='01' order by a.com_id,a.proj_id ");
            } else
            {
                sql.delete(0, sql.length());
...
" "
    ...
    public Hashtable getDocHeaderDetails(String iDocNo)
    {
        ...
        try
        {
            stmt = conn.createStatement();
            sql.delete(0, sql.length());
            sql.append(" select trim(c.n_prename_th)||trim(c.n_nemploy_th)||' '||trim(c.n_semploy_th) emp_name , ")
                .append(" b.i_company||'-'||b.i_project||' '||b.n_project sel_project,d.i_tel ,f.ven_name ,to_char(a.d_appoint_cust,'%H:%M') as time, a.* ")
                .append(" from lan:serv_dochd a ")
                .append(" left join lan:acxprojt b on b.i_company=a.i_company and b.i_project=a.i_project")
				.append(" left join lan:serv_prjdt d on d.i_company=a.i_company and d.i_project=a.i_project")
				.append(" left join lan:unit e on e.i_company=a.i_company and e.i_project=a.i_project and ")
				.append(" e.i_lock=a.i_lock and e.unit_status='OPN' ")
				.append(" left join lan:vendor f on f.ven_no=e.ven_no  ")
				.append(" left join docflow:acemploy c on c.i_employ=a.i_service_employ  ")
            ...
            if(rs.next())
            {
				...
				result.put("i_warranty", doString.checkString(rs.getString("i_warranty"), ""));
				//modify by pradoem 2019.12.03
				result.put("time", doString.checkString(rs.getString("time"), ""));
				//modify by pradoem 2021.03.15
				result.put("i_system", doString.checkString(rs.getString("i_system"), ""));
            }
            rs.close();
            stmt.close();
        }catch(Exception e){
    ...
    public Hashtable getInfDocHeaderDetails(String iDocNo)
    {
        ...
        try
        {
            ...
            sql.delete(0, sql.length());
            sql.append(" select trim(c.n_prename_th)||trim(c.n_nemploy_th)||' '||trim(c.n_semploy_th) emp_name , ")
            .append(" b.i_company||'-'||b.i_project||' '||b.n_project sel_project,d.i_tel ,a.* ")
            .append(" from lan:serv_infdochd a ")
            .append(" left join lan:acxprojt b on b.i_company=a.i_company and b.i_project=a.i_project ")
            .append(" left join lan:serv_prjdt d on d.i_company=a.i_company and d.i_project=a.i_project ")
            .append(" left join docflow:acemploy c on c.i_employ=a.i_service_employ  ")
            .append(" where a.i_docno='").append(iDocNo).append("'");
    ...
    public Hashtable getCustomerDetails(String iCompany, String iProject, String iLock, String iHouse)
    {
        ...
        try
        {
            ...
            .append(" and b.i_lor=a.i_lor and b.f_contr is null ").append(" where a.i_company='").append(iCompany).append("' ")
            ...
            if(rs.next()) {
                ...
                nCustTel = doString.checkString(rs.getString("a_id_tel"), "");
            ...
            //2019.06.19 modify by pradoem
			SVC_CUSTOMER objCust = FetchesSvcCustomer(conn, iCompany, iProject, iLock, iHouse);
			if(objCust!=null) {
				//obj.setPrefixName("");
				//obj.setFName(objCust.getCustFname());
				//obj.setLName(objCust.getCustLname());
				//obj.setTelNo(objCust.getCustTel());
				//obj.setStatus("E");
				custName = objCust.getCustFname()+"  "+objCust.getCustLname();
				result.put("n_customer", custName);
				nCustTel = objCust.getCustTel();
				result.put("n_cust_tel", nCustTel);
				
				//System.out.println("11111111:   "+objCust.getCustFname());
				//System.out.println("22222222:   "+objCust.getCustLname());
				//System.out.println("33333333:   "+objCust.getCustTel());
			}else{
				//System.out.println("===>Get SVC_CUSTOMER is null ");	
			} 
        }
        catch(Exception e){
    ...
    public String genWrongTypeCutList(String name, String value, String params)
    {
        StringBuffer html = new StringBuffer();
        StringBuffer sql = new StringBuffer();
        Statement stmt = null;
        ...
        {
            stmt = conn.createStatement();
            sql.append(" select * from lan:serv_xstd where i_type='10' ");
            rs = stmt.executeQuery(sql.toString());
            html.append("<select name='").append(name).append("' ").append(params).append(" >");
			html.append("<option value=''>- เลือก -</option>");	
            String code;
	...
	public SVC_CUSTOMER FetchesSvcCustomer(Connection conn, String comId, String projId, String lockNo,String houseNo) {
		StringBuffer sql = new StringBuffer();	
		PreparedStatement pstmt = null;
		ResultSet rs = null;
        try{
        	//initial parameter
        	SVC_CUSTOMER  obj  = null; 
        	boolean isAvailable = false;
        	//System.out.println("FetchesSvcCustomer ->Starting.");        	 
			/******************************************************/			
        	//*****Find project by user login  
			sql.delete(0,sql.length());
			sql.append(" select * from lan:svc_customer ")
				  .append(" Where 1 = 1  ")
			...
			if(rs.next()){
				isAvailable = true;
				obj = new SVC_CUSTOMER();
				obj.setRecId(doString.checkString(rs.getString("rec_id"), ""));
				obj.setCompanyId(doString.checkString(rs.getString("i_company"), ""));
				obj.setProjectId(doString.checkString(rs.getString("i_project"), ""));
				obj.setLockId(doString.checkString(rs.getString("i_lock"), ""));
				obj.setHouseId(doString.checkString(rs.getString("i_house"), ""));
				obj.setModel(doString.checkString(rs.getString("i_model"), ""));
				obj.setiLor(doString.checkString(rs.getString("i_lor"), ""));
				obj.setdCloseLaw(doString.checkString(rs.getString("d_close_law"), ""));
				obj.setCustomerId(doString.checkString(rs.getString("i_customer_id"), ""));
				obj.setCustFnameOld(doString.checkString(rs.getString("c_fname_old"), ""));
				obj.setCustLnameOld(doString.checkString(rs.getString("c_lname_old"), ""));
				obj.setCustTelOld(doString.checkString(rs.getString("c_tel_old"), ""));
				obj.setCustFname(doString.checkString(rs.getString("c_fname"), ""));
				obj.setCustLname(doString.checkString(rs.getString("c_lname"), ""));
				obj.setCustTel(doString.checkString(rs.getString("c_tel"), ""));
				obj.setCustMobile(doString.checkString(rs.getString("c_mobile"), ""));
				obj.setEmail(doString.checkString(rs.getString("c_email"), ""));
				obj.setLineId(doString.checkString(rs.getString("c_line_id"), ""));
				obj.setOther(doString.checkString(rs.getString("c_other"), ""));
				obj.setAddr1(doString.checkString(rs.getString("addr1"), ""));
				obj.setAddr2(doString.checkString(rs.getString("addr2"), ""));
				obj.setAddr3(doString.checkString(rs.getString("addr3"), ""));
				obj.setAddr4(doString.checkString(rs.getString("addr4"), ""));
				obj.setAddr5(doString.checkString(rs.getString("addr5"), ""));
				obj.setRemark(doString.checkString(rs.getString("c_remark"), ""));
				obj.setStatus(doString.checkString(rs.getString("status"), ""));
				obj.setdCreate(doString.checkString(rs.getString("d_create"), ""));
				obj.setdUpdate(doString.checkString(rs.getString("d_update"), ""));
				obj.setUsrCreate(doString.checkString(rs.getString("usr_createby"), ""));
				obj.setUsrUpdate(doString.checkString(rs.getString("usr_updateby"), ""));
			}
			rs.close();	
			//********************************************************/		
			//System.out.println("FetchesSvcCustomer ->successfully. ");				  	 
		  	return obj;			  	 
		}catch(Exception e){
			System.out.println("!!!FetchesSvcCustomer , SERV_CommonData " + e.getMessage());
