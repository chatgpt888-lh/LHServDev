...
String jName = "SERV_WageLaborMnt.jsp";
//ServLog servlog = new ServLog(sessionId, userId, jName);
	...
	try {
		...
		double sum_date = 0, tot_holiday = 0, holiday = 0;
...
<style type="text/css">
TD				{ 	font-size:8pt ; font-family : Microsoft Sans Serif ; color : rgb(0,0,0) ; mso-number-format:"\@";		}
...
function submit()  {		
		Wage.action = "/LHServ/SERV_WageLaborMnt.jsp";
		Wage.submit();
}
...
<FORM NAME="Wage" METHOD="POST" ACTION="SERV_WageLaborMnt.jsp">
...
<table border="0" cellpadding="0" cellspacing="0" width="1270px">
 <tr height="30">
  <td width="100%" class="bigh" colspan="2">ตารางสรุปการเข้างานของธุรการ, วิศวกร : โครงการ<%=n_project%></td>
 </tr>
 <tr height="30">
...
<table width="1270px" border="0" cellspacing="0" cellpadding="0">
  <tr>
	...
	<td class="col_nameREP" style="border-top:1px solid rgb(135,185,247)" rowspan="2">โครงการ</td>
    <td class="col_nameREP" style="border-top:1px solid rgb(135,185,247)" rowspan="2">ชื่อ-สกุล</td>
	<td class="col_nameREP" style="border-top:1px solid rgb(135,185,247)" rowspan="2">ผู้รับเหมา</td>
    <td class="col_nameREP" style="border-top:1px solid rgb(135,185,247)" rowspan="2">เลขที่บัตรประชาชน</td>
    <td class="col_nameREP" style="border-top:1px solid rgb(135,185,247)" rowspan="2">เงินเดือน</td>
...
<%
   ...
   sql.delete(0,sql.length());	
   sql.append("select distinct a.i_name, a.i_cardno, a.z_wage, a.n_job, a.i_header ")
		.append("from lan:serv_tstaff a, lan:serv_finger b ")
		.append("where a.i_header = b.i_header ")
		.append("and a.i_header is not null ")
		.append("and a.i_cardno = b.i_cardno ");
	if (!project.equals("LHALL")) {
	sql.append("and a.i_header = '"+project+"' ");
	}		
   sql.append("and b.i_date between '"+d_start+"' and '"+d_stop+"' ")
	    .append("and a.z_wage > 10000 ")
		.append("order by a.i_header, a.n_job, a.i_name ");
	
   /*sql.delete(0,sql.length());	
   sql.append("select distinct i_name, i_cardno, z_wage, n_job, i_header ")
		.append("from lan:serv_tstaff ")
		.append("where i_header is not null ")
	    .append("and z_wage > 10000 ");
if (!project.equals("LHALL")) {
	sql.append("and i_header = '"+project+"' ");
}
	sql.append("order by i_header, n_job, i_name ");		*/	
    ...
    while (rs.next()) {
	   ...
	   i_com = doString.checkString(rs.getString("i_header").substring(0,2));
	   i_proj = doString.checkString(rs.getString("i_header").substring(2,5));

		
		sql.delete(0,sql.length());
 		sql.append("select i_vendor1 from lan:serv_lstaff ")
			 .append("where i_company = '"+i_com+"' ")
			 .append("and i_project = '"+i_proj+"' ");
		System.out.println(sql.toString());			
 		rs1 = stmt1.executeQuery(sql.toString());
 		if(rs1.next()){ 			
 			i_vendor1 = doString.checkString(rs1.getString("i_vendor1"));
		}
		...
		sql.delete(0,sql.length());
 		...
 		rs1 = stmt1.executeQuery(sql.toString());
		if (rs1.next()) {
			n_vendor = doString.checkString(doString.DisplayThai(rs1.getString("bus_name")));
		}
%>

    <tr>
    <td class="dotlineREP" style="border-left:1px solid rgb(135,185,247)" align="center"><%=no%></td>
	<td class="dotlineREP" align="center"><%=doString.checkString(rs.getString("i_header"))%></td>
    <td class="dotlineREP ; item"><A HREF="SERV_WageViewEdit.jsp?card_no=<%=doString.checkString(rs.getString("i_cardno"))%>&YY=<%=YY%>&YY1=<%=YY1%>&MM=<%=MM%>&MM1=<%=MM1%>&day1=<%=day1%>&project=<%=project%>"><%=doString.checkString(doString.DisplayThai(rs.getString("i_name")))%></A>&nbsp;</td>
	 <td class="dotlineREP" align="left"><%=n_vendor%></td>
 ...
 <%    for (int dd = 21;dd <= day1;dd++) {   
   ...
   sql.append("select distinct i_name, i_cardno, i_date, i_checkin, i_checkout ")
		...
		.append("where month(i_date) = '"+MM1+"' ")
		.append("and year(i_date) = '"+(YY1-543)+"' ")
	    .append("and day(i_date) = '"+dd+"' ")
	    .append("and i_cardno = '"+doString.checkString(rs.getString("i_cardno"))+"' ");
   if (!project.equals("LHALL")) {
	   	sql.append("and i_header = '"+doString.checkString(rs.getString("i_header"))+"' ");
   }
		sql.append("order by i_name ");
   System.out.println(sql.toString());			
   rs1 = stmt1.executeQuery(sql.toString());					
   if (rs1.next()==true) {
			//img = "images/i_pass3.gif";
			...
			//2015-08-02
			if (!i_date.equals("-")) {
					chk_date = Integer.toString(Integer.parseInt(i_date.substring(0, 4))+543)+"-"+i_date.substring(5,7)+"-"+i_date.substring(8,10);					
			}	
				   sql.delete(0,sql.length());	
...
//--------------------- คำนวนวันหยุด ----------------------
	   ...
	   sql.delete(0,sql.length());	
	   sql.append("select tot_holiday from lan:serv_holiday ")
			.append("where i_type = '01' ")
			.append("and i_month = '"+MM+"' ")
			.append("and i_year = '"+YY+"' ");				
		rs2 = stmt2.executeQuery(sql.toString());					
		if (rs2.next()==true) {
				holiday = rs2.getInt("tot_holiday");
		}

if (z_wage > 10000 & days > 0) {	
		sum_date = 30 - holiday;	 	
		if (days <= sum_date) {
...
if (z_wage > 10000 & days > 0) {
		...
		sum_date = 30 - days;	 // จำนวนวันที่ไม่ได้มาสแกนนิ้ว
		if (sum_date <= holiday) {
			sum_wage = z_wage;
		} else {
			tot_holiday = sum_date - holiday;   // คำนวนวันหยุดที่หักจากวันหยุดประจำ		
			sum_wage = (z_wage/30) * (days+tot_holiday);   
		}		
...
%>
    ...
    <tr>
    <td class="dotlineREP ; bold" style="border-left:1px solid rgb(135,185,247)" align="right" colspan="<%=q_day+26%>">รวมเป็นเงิน</td>
    <td class="dotlineREP" align="right" colspan="2"><%=doString.displayNumber("#,###,##0.00",totsum_wage)%></td>
    <td class="dotlineREP">บาท</td>
    </tr> 
    <tr>
    <td class="dotlineREP ; bold" style="border-left:1px solid rgb(135,185,247)" align="right" colspan="<%=q_day+26%>">ค่าดำเนินการ 17%</td>
    <td class="dotlineREP" align="right" colspan="2"><%=doString.displayNumber("#,###,##0.00",rate)%></td>
    <td class="dotlineREP">บาท</td>
    </tr> 
    <tr>
    <td class="solidlineREP01 ; bold" style="border-left:1px solid rgb(135,185,247)" align="right" colspan="<%=q_day+26%>">รวมเป็นเงินทั้งสิ้น</td>
...
<%
		...
		System.out.println("ERROR SERV_WageLaborMnt.jsp : " + e.getMessage());
