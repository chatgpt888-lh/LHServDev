 ...
 * @author
 */
public class SERV_PrintImageServlet extends DBServlet  {
...
/*
		...
		ResultSet rs1 = null;
		 ...
		 try {
			...
			while (rs.next()) {
				nVendor = doString.checkString(rs.getString("bus_name"),"");
				itemStatus = doString.checkString(rs.getString("f_itmstatus"),"");				
				
				if (itemStatus.equals("100")) {	
					dJob = getDateValue(rs.getTimestamp("d_approve"));
				} else if (itemStatus.equals("200")) {
					dStartTask = getDateValue(rs.getTimestamp("d_approve"));
				} else if (itemStatus.equals("300")) {
					dComplete = getDateValue(rs.getTimestamp("d_approve"));
				}
			} // end while
			rs.close();
			*/
			...
			sql.delete(0,sql.length());
			sql.append(" select * from lan:serv_payment where i_docno='").append(iDocNo).append("' ")
			...
			while (rs.next()) {		
					...
					/*
					 * Remark by pradoem 2015.04.29
					 * sql.delete(0,sql.length());
					sql.append("select * from lan:serv_docatt where i_docno='").append(iDocNo).append("' ")
					      .append(" and i_seq='"+doString.checkString(rs.getString("i_seq"),"")+"' ")
						  .append(" and i_itmjob='"+doString.checkString(rs.getString("i_itmjob"),"")+"' ")
						  .append(" and i_itmjob_area='"+doString.checkString(rs.getString("i_itmjob_area"),"")+"' ");	  */
					...
					if (
						before[0].length()>0 || before[1].length()>0 ||  // found before image
						...
						//-- line 2 --//
						cell = setCellAttribute("ลำดับที่ : "+doString.checkString(rs.getString("i_seq"),""),microssfont,"L","T",60,0);
						table.addCell(cell);							
						cell = setCellAttribute("",microssfont,"L","T",10,0);
						table.addCell(cell);					
