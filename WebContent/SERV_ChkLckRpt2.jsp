			...
			<tr>
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td width="5%" class="col_nameO">แปลงขาย</td>
						<td width="10%" class="col_name">วัน/เวลานัด Check up</td>
						<td width="6%" class="col_name">สถานะ</td>
						<td width="11%" class="col_name">ร้านค้าแอร์</td>						
						<td width="10%" class="col_name">ร้านค้าปลวก</td>												
						<td width="13%" class="col_name">ผรม.ซ่อม</td>
						<td width="10%" class="col_name">ชื่อลูกค้า</td>
						<td width="10%" class="col_name">เบอร์โทรศัพท์</td>
						<td width="6%" class="col_name">วันที่โอน</td>
					</tr>
<%
	String time = "";
	int seqNo = 0;
	...
	if (rsChkup != null) {
		while (rsChkup.next() == true) {
			...
			air = "";			
			rs = stmt.executeQuery("SELECT v.ven_name FROM lan:serv_venprj p, lan:vendor v WHERE p.i_company = '"+comId+"' AND p.i_project = '"+projId+"' AND p.i_type = '03' AND p.i_vendor = v.ven_no");
			if (rs != null) {
				if (rs.next() == true) {
					air = doString.checkString(rs.getString(1));
				}
				rs.close();
				rs=null;
			}			
			ant = "";			
			rs = stmt.executeQuery("SELECT v.ven_name FROM lan:serv_venprj p, lan:vendor v WHERE p.i_company = '"+comId+"' AND p.i_project = '"+projId+"' AND p.i_type = '04' AND p.i_vendor = v.ven_no");
			if (rs != null) {
				if (rs.next() == true) {
					ant = doString.checkString(rs.getString(1));
				}
				rs.close();
				rs=null;
			}			
...
%>
					<tr>
						...
						<td width="15%" align="left" class="dotline"><%=comId%><%=projId%> <%=doString.DisplayThai(project)%></td>
						<td width="5%" class="dotline" align="center"><%=lockId%></td>
						<td width="10%" class="dotline" align="center"><%=doString.checkString(rsChkup.getString("I_DAY"))%>/<%=doString.DisplayThai(time)%></td>
						<td width="6%" class="dotline" align="center"><%=doString.DisplayThai(rsChkup.getString("N_STATUS"))%></td>						
						<td width="11%" class="dotline" align="center"><%=doString.DisplayThai(air)%></td>							
						<td width="10%" class="dotline" align="center"><%=doString.DisplayThai(ant)%></td>													
						<td width="13%" class="dotline" align="left"><%=doString.DisplayThai(rsChkup.getString("VEN_NAME"))%>&nbsp;</td>						
						...
						<td width="6%" align="center" class="dotline"><%=DateUtil.ifxToThaiDateNoTime(rsChkup.getString("D_CLOSE_LAW"))%></td>
					</tr>
<%		
		}// end while
		rsChkup.close();
