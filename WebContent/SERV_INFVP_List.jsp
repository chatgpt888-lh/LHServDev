...
<!--
  ...
  function searchDocHD() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_INFVP_List.jsp";
...
<input type="hidden" name="success_page" value="SERV_INFVP_List.jsp?itmType=<%=itmType%>&d_payment=<%=dPayment%>">
<input type="hidden" name="error_page" value="SERV_INFVP_List.jsp?error=1&itmType=<%=itmType%>&d_payment=<%=dPayment%>">
                      ...
                      while (rs.next()) {
					        ...
					        %>
							  <tr>
							    ...
							    <td width="30%" class="dotline" align="left"><a href="SERV_INFVP_List2.jsp?itmType=<%=itmType%>&i_company=<%=iCompany%>&d_payment=<%=dPayment%>"><%=iCompany%> - <%=companyName%></a></td>
...
<%
       System.out.println("----  SERV_INFVP_List.jsp ---- ");
	} catch (Exception e) {
		System.out.println("ERROR SERV_INFVP_List.jsp : " + e.getMessage());
