...
<%!
        ...
        throws Exception
    {
        ...
        //for(int i = nowYear - 15; i <= nowYear + 2; i++)
        for(int i = 2007; i <= nowYear + 2; i++) // 2023-01-30 , fixed start year = 2007 (2550)
        {
...
String jName = "SERV_ScrRetDet2.jsp";
...
<%
	 ...
	 //	   .append(" where a.i_budg_type in (1,2)  ")
	 //	   .append(" order by a.i_company , a.i_project ");    	 
     sql.delete(0,sql.length()); 
	 sql.append(" select * from lan:acxprojt ")
	    .append(" where d_confirm is not null and i_project<>'099' and i_project<>'0' ")
...
function clearSelProj() {
  ...
  function searchRetDoc() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_RepRetDet2.jsp";
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	...
	<select size="1" class="box" style="width:400px" name="i_company" onchange="filterProject();">
		...
		<%
			...
			while (rs.next()) {
				 ...
				 comDesc = doString.checkString(doString.DisplayThai(rs.getString("n_company")),"");
				 
				 sel = "";
				 if (iCompany.equals(comId)) {
				 	sel = "selected";
				 }
				 
				 %><option value='<%=comId%>' <%=sel %>><%=comId+" | "+comDesc%></option><%
			}
			rs.close();
		%>
     </select>
  ...
  <tr>
    <td class="item ; dotline01" height="22" width="18%">วันที่ Payin
      :</td>
    <td height="22" width="82%" class="dotline01">
          <%=genDateListbox("start",request," class='box' ")%>
          &nbsp; &nbsp; ถึง : &nbsp; &nbsp;
          <%=genDateListbox("end",request," class='box' ")%>
	</td>
  </tr>
  <tr>
    <td class="item ; dotline01" height="22" width="18%">รูปแบบรายงาน
      :</td>
    <td height="22" width="82%" class="dotline01">
          <input type="radio" class="box" name="report_type" value="REMAIN" <%=reportType.equalsIgnoreCase("REMAIN") ? "checked" : "" %>> &nbsp; สรุปยอดคงค้างการวางเงินค้ำประกัน (แสดงเฉพาะรายการที่ยังไม่คืนเงิน)
          &nbsp; &nbsp; 
          <input type="radio" class="box" name="report_type" value="ALL" <%=reportType.equalsIgnoreCase("ALL") ? "checked" : "" %>> &nbsp; รายละเอียดการวางเงินค้ำประกัน (แสดงทุกรายการ)
	</td>
  </tr>  
</table>
...
<%
	if (iCompany.trim().length()>0) {
		%>
		<script>filterProject();</script>
		<%
	}
%>
...
<%
		...
		System.out.println("ERROR SERV_ScrRetDet2.jsp : " + e.getMessage());
