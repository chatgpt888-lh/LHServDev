...
String jName = "SERV_Zone_List.jsp";
...
<!--
  ...
  function searchDocHD() {
     ...
     document.forms[0].action="<%=Constants.APP_PATH%>/SERV_Zone_List.jsp";
...
<input type="hidden" name="success_page" value="SERV_Zone_List.jsp?d_payment=<%=dPayment%>">
<input type="hidden" name="error_page" value="SERV_Zone_List.jsp?error=1&d_payment=<%=dPayment%>">
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td width="5%" class="col_name" rowspan="2">จำนวน<br>ใบแจ้งซ่อม</td>
			...
			if(user.getUserWho().equals("A")){ //lee admin
		     ...
		     //----================== Select Data from SERV_DOCHD ================----//   
		        ...
		        sql.append(" select distinct d.bus_name,b.i_vendor ")
		              ...
		              .append(" ,lan:serv_dochd a where b.i_docno=a.i_docno and a.f_status='OPN' and b.f_itmstatus='700' ")
		              .append(condition)
				...
				servlog.endLog();
                      ...
                      while (rs.next()) {
						   ...
						   //---=========================================================================----//      
							...
							//----====================== Count Job Form ==================----//
							sql.delete(0,sql.length());
							sql.append(" select count(*) from serv_payment b,serv_dochd a where b.f_itmstatus='700' ")
					        ...
					        %>
							  <tr>
							    ...
							    <td width="30%" class="dotline" align="left"><a href="SERV_Zone_Conf.jsp?i_vendor=<%=iVendor%>"><%=vendorName+"&nbsp; "+markupPay%></a></td>
...
<%           
      System.out.println("---- SERV_Zone_List.jsp ----- ");
	} catch (Exception e) {
