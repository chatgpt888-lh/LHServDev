...
String jName = "SERV_Apprv_RetReten.jsp";
ServLog servlog = new ServLog(sessionId, userId, jName);
   ...
   String condition = "";
	...
	sql.append(" select b.i_company||b.i_project||' | '||b.n_project as project_name , c.n_desc , e.c_apprv , f.c_apprv as  c_prev_apprv , ")
	      ...
	      //-----------------------------------------//	  	      
	      .append(" where a.i_doc_status in ('O','V') and a.z_reten=a.z_recv_reten and a.i_staff_payback is not null ")
...
<table cellspacing="0" cellpadding="0" width="100%">
                                  <tr>
                                    ...
                                    <td>
				    <%
				    if (iDocStatus.equals("O")) {
				        String curApprv[] = getEmployeeDetails(stmt1,iCurApprove);
					...
					sql.append(" select i_apprv from lan:serv_apprv where ")
					      ...
					      .append(" and i_doc_status='O' ");
						servlog.startLog(sql.toString());
						rs = stmt.executeQuery(sql.toString());
						servlog.endLog();
					if (rs.next()) {
					    String iEmploy = doString.checkString(rs.getString("i_apprv"),"");
					    String curApprv[] = getEmployeeDetails(stmt1,iEmploy);
					    out.println("<font color='#000096'>"+doString.DisplayThai(curApprv[1])+"</font> &nbsp; ( "+doString.DisplayThai(curApprv[2])+" )");
					} // end while rs
					rs.close();
				    }
