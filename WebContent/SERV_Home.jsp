 ...
 */ 
  ...
  public String getDateFromResultSet(ResultSet rs,String fieldName) throws Exception {
	...
	public String XXgenProjectListboxByUserId(Connection conn,String userId,String name,String value,String params,boolean getAllProj) {
		 StringBuffer html = new StringBuffer();
		 StringBuffer sql = new StringBuffer();
		Statement stmt = null;
		 ResultSet rs = null;
		 ...
		 try {
			...
			if (userWho.equalsIgnoreCase(Constants.PERMISSION_VENDOR)) {
			 ...
			 //System.out.println(" value :"+sql.toString());
			 rs = stmt.executeQuery(sql.toString());
			 //-------============== Generate List box ===================------//
			 html.append("<select name='").append(name).append("' ").append(params).append(" >");
			 html.append("<option value=''>"+Constants.LISTBOX_SELECT_LABEL+"</option>");
...
ServLog servlog = new ServLog(sessionId, userId, jName);
   ...
   //-----====================== Search BOQ Data ================================------//
	...
	try {
		...
		boolean isSvc = IsGrantPermissionSvc(conn,empId);
	    boolean isLHALL = IsLHALL(conn,user.getUserID(),selProj);
		System.out.println("isLHALL = "+isLHALL);
...
/*		if (selProj.trim().length()<=0) {
		   ...
		   } else if (selProj.equalsIgnoreCase("ALL_PROJ"))  { //ALL_PROJ
...
*/
       ...
       //remark by pradoem 2012.04.24 : sql.append(" select * from lan:serv_payschd where d_change>=today order by d_payment ");
		...
		if (!user.getUserWho().equalsIgnoreCase(Constants.PERMISSION_VENDOR)) {
			//System.out.println("------------->Test :"+userId);
                ...
                if(viewAllStaffProj){
                      ...
                      sql.append(" select count(*) from lan:serv_pstaff a,lan:eser_dochd b ")
                      ...
                      if(isLHALL){
                       ...
                       sql.append(" select count(*) from lan:serv_pstaff a,lan:eser_dochd b ")
                        ...
                        .append(" AND a.d_keyin >= ADD_MONTHS(SYSDATE, -12) ")//fix by petch
                      	...
                      	//all by admin              
                        sql.append(" select count(*) from lan:eser_dochd a where a.f_status='OPN' ")
                         .append(condition)
                         .append(" AND a.d_keyin >= ADD_MONTHS(SYSDATE, -12) ")//fix by petch
              			 ;
                      }
               ...
               System.out.println("------------->SQL eServCount :"+sql.toString());  
               eServCount = getCountValue(stmt,sql.toString(),servlog);
		
			//------ if employee not vendor , check job ---------//
			//System.out.println(" viewAllStaffProj :"+sql.toString());
			sql.delete(0,sql.length());
			if (viewAllStaffProj) {
				//---- query for staff view all project ----//
			...
			//System.out.println(" jobWait sql.toString() :"+sql.toString());
			//System.out.println(" isSvc :"+isSvc);
			//System.out.println(" user_group :"+user_group);
			if(!isSvc && !user_group.equals("I")){
               
                //System.out.println(" if(!isSvc && !user_group.equals(I)){ sql.toString() :"+sql.toString());
				jobWait = getCountValue(stmt,sql.toString(),servlog); 
				//System.out.println(" if(!isSvc && !user_group.equals(I)){ viewAllStaffProj :"+viewAllStaffProj);
				//System.out.println(" if(!isSvc && !user_group.equals(I)){ userWho :"+user.getUserWho());
				//System.out.println(" if(!isSvc && !user_group.equals(I)){ userAcl :"+user.getUserACL());
                //System.out.println("  if(!isSvc && !user_group.equals(I)){ jobWait :"+jobWait);
				...
				//System.out.println("sql 555 taskPass,completeWait :"+sql.toString());
				rs = stmt.executeQuery(sql.toString());
				while (rs.next()) {
				...
				//System.out.println("completePass :"+sql.toString());
				completePass = countResultRow(stmt,sql.toString(),servlog);
				//System.out.println("taskWait :"+taskWait);
				//System.out.println("completeWait :"+completeWait);

			} //End check is if(!isSvc)
          }// end if check vendor
		...
		if(!isSvc){
			...
			/*
			for สำหรับ Admin,user,AVP,VP ดูแล role บ้านเท่านั้น
			*/
			if (user_group.equals("A") || user_group.equals("H")) {
			    //System.out.println(" Main SQL House(Home) = "+sql.toString());
				rs = stmt.executeQuery(sql.toString());
				while (rs.next()) {
					itemStatus = doString.checkString(rs.getString("f_itmstatus"),"");
  ...
  //System.out.println("---->cVPWait = "+cVPWait);
        ...
        int cCONConstructorWait = 0;
        int cCONConstructorPass = 0;
        int cCONConstructorReject = 0;
        int cCONServiceStaffWait = 0;
        int cCONServiceStaffPass = 0;
        int cCONServiceManagerWait = 0;
        int cCONServiceManagerPass = 0;        
        int cCONManagerWait = 0;
        int cCONManagerPass = 0;              
        int cCONVPWait = 0;
        int cCONVPPass = 0;
        
        int nCONConstructorWait = 0;
        int nCONConstructorPass = 0;
        int nCONConstructorReject = 0;
        int nCONServiceStaffWait = 0;
        int nCONServiceStaffPass = 0;
        int nCONServiceManagerWait = 0;
        int nCONServiceManagerPass = 0;        
        int nCONManagerWait = 0;
        int nCONManagerPass = 0;              
        int nCONVPWait = 0;
        int nCONVPPass = 0;
		...
		if(!isSvc){		
			...
			if (viewAllStaffProj) {
				...
				sql.append(" select count(distinct c.i_docno) as cnt, c.f_itmstatus, b.f_reject, b.d_reject, c.d_payment ")
					  ...
					  .append(" where b.f_status != 'CAN' and a.user_id = '").append(userId).append("' ")
					  .append(" and a.com_id = b.i_company and a.proj_id = b.i_project ")
					  .append(" and b.i_doc_type = 'C' and b.i_docno = c.i_docno ")
					  .append(" and c.i_itmtype = '03' and (c.d_payment = '"+currentPaymentDate+"' or c.d_payment = '"+nextPaymentDate+"') ")
					  .append(" and c.f_itmstatus != 'CAN' ")
					  .append("  group by c.f_itmstatus, b.f_reject, b.d_reject, c.d_payment  ");
			} else {
			...
			/* if(isLHALL){ //isLHALL==true
				...
				sql.append(" select count(distinct b.i_docno) as cnt ,b.f_itmstatus,a.f_reject,a.d_reject,b.d_payment ")
					  .append(" from lan:serv_infdochd a,lan:serv_infpayment b where a.f_status!='CAN' and a.i_doc_type='C' ")
					  .append(" and b.f_itmstatus!='CAN' and b.i_docno=a.i_docno and b.i_itmtype = '03' "+condition+" ")
					  .append(" and (b.d_payment='"+currentPaymentDate+"' or b.d_payment='"+nextPaymentDate+"') ")
					  .append(" group by b.f_itmstatus,a.f_reject,a.d_reject,b.d_payment ");
			 }*/
			...
			if (user_group.equals("A") || user_group.equals("I")) {
				...
				while (rs.next()) {
					...
					if (itemStatus.equalsIgnoreCase("400")) {
						if (fReject.equalsIgnoreCase("N") && dReject.trim().length()>0) {
							if (currentPaymentDate.equals(dPayment)) {
								cCONConstructorReject += count; // Constructor Reject , current payment
							} else if (nextPaymentDate.equals(dPayment)) {
								nCONConstructorReject += count; // Constructor Reject , next payment
							}
						} else {
							if (currentPaymentDate.equals(dPayment)) {
								cCONConstructorWait += count; // Constructor Wait , current payment
							} else if (nextPaymentDate.equals(dPayment)) {
								nCONConstructorWait += count; // Constructor Wait , next payment
							}
						}
					} 
					else if (itemStatus.equalsIgnoreCase("500")) {
						if (currentPaymentDate.equals(dPayment)) {
							cCONConstructorPass += count; // Constructor Pass , current payment
							cCONServiceStaffWait += count; // Service Staff Wait , current payment
						} else if (nextPaymentDate.equals(dPayment)) {
							nCONConstructorPass += count; // Constructor Pass , next payment
							nCONServiceStaffWait += count; // Service Staff Wait , next payment
						}
					}
					else if (itemStatus.equalsIgnoreCase("600")) {
						if (currentPaymentDate.equals(dPayment)) {
							cCONServiceStaffPass += count; // Service Staff Pass , current payment
							cCONServiceManagerWait += count; // Service Managet Wait , current payment
						} else if (nextPaymentDate.equals(dPayment)) {
							nCONServiceStaffPass += count; // Service Staff Pass , next payment
							nCONServiceManagerWait += count; // Service Managet Wait , next payment
						}
					}
					else if (itemStatus.equalsIgnoreCase("700")) {
							if (currentPaymentDate.equals(dPayment)) {
								cCONServiceManagerPass += count; // Service Manager Pass , current payment
								cCONManagerWait += count; // Manager Wait , current payment
							} else if (nextPaymentDate.equals(dPayment)) {
								nCONServiceManagerPass += count; // Service Manager Pass , next payment
								nCONManagerWait += count; // Manager Wait , next payment
							}
					}
		
					else if (itemStatus.equalsIgnoreCase("800")) {
							if (currentPaymentDate.equals(dPayment)) {
								cCONManagerPass += count; // Managet Pass , current payment
								cCONVPWait += count;  // VP Wait , current payment 
							} else if (nextPaymentDate.equals(dPayment)) {
								nCONManagerPass += count; // Managet Pass , next payment
								nCONVPWait += count;  // VP Wait , next payment 
							}
					}
		
					else if (itemStatus.equalsIgnoreCase("CLS")) {
							if (currentPaymentDate.equals(dPayment)) {
								cCONVPPass += count;  // VP Pass , current payment 
							} else if (nextPaymentDate.equals(dPayment)) {
								nCONVPPass += count;  // VP Pass , next payment 
							}
					}
				}
				rs.close();
...
<script language="javascript">
  ...
  function onPleaseWait(){
	...
	//setTimeout(function(){  $('#pleasewaitScreen').css("visibility", 'hidden'); }, 9000000); //wait 7 seconds
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    <td width="100%" align="center" class="BD">
	  ...
	  <%
	   if (user.getUserWho().equals("C") || !user.getUserWho().equals("V") && !user.getUserWho().equals("G") && serv_center.equals("Y") || userId.equals("wiranid")) {   //25/03/2015
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
   ...
   <%--  modify by pradoem 2012/03/13 --%>
   <tr>
    <td width="55%" class="item ; dotline" align="left"><img border="0" src="images/i_arrow1.gif" align="absmiddle" width="13" height="13">Line Service/E-Service รอเปิด Inform Job</td>
    <td width="15%" class="dotline" align="center">

    	<a href="#" onclick="javascript:doSubmitForm('ESERV_OpenJob_List.jsp?criteria=<%=param %>&sel_project=<%=selProj%>');"><%=eServCount%></a>
  ...
  <tr>
    ...
    <td width="15%" class="dotline" align="center">
    	...
    	<a href="#" onclick="javascript:doSubmitForm('SERV_OpenJob_List.jsp?sel_project=<%=selProj%>');"><%=jobWait%></a>

    	<%}else{out.println(jobWait);} %>
    ...
    <%if(taskWait>0){ %>
    	...
    	<a href="#" onclick="javascript:doSubmitForm('SERV_StartTask_List.jsp?sel_project=<%=selProj%>');"><%=taskWait%></a>

    <%}else{out.println(taskWait);} %>	
  ...
  <tr>
    ...
    <%if(completeWait>0){ %>
    	...
    	<a href="#" onclick="javascript:doSubmitForm('SERV_CompTask_List.jsp?sel_project=<%=selProj%>');"><%=completeWait%></a>

    <%}else{out.println(completeWait);} %>
...
//System.out.println("---6666------");
	if (selProj.equals("")) {
		inf_condition = " AND h.i_project = 'NOPROJECT'";
	} else {
	   if (selProj.equals("ALL")) {
	      
	   } else {
	        if(selProj.trim().length()>=6){ //fix by pradoem 2023.02.21  LH:075
	        	   inf_condition = " AND h.i_company = '"+selProj.substring(0,2)+"' AND h.i_project='"+selProj.substring(3,6)+"'";
	        }
	   }
	}
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          <td width="40%" class="bigh">&nbsp;</td>
			...
			<a href="/LHServ/ServOpenJob?actionMode=initForm&i_itmtype=02"><img border="0" src="images/icon_open_JopPUB.gif" width="120" height="34"></a> 
            </td>
        </tr>
<br style="font-size:10pt">
      </table>
            ...
            <td class="item_tab2" width="200">รายละเอียดงานสั่งซ่อมสาธารณะ</td>
                <td class="item_tab3"></td>
                <td >&nbsp;</td>
              </tr>
            </table>
...
<%
	...
	sql.append("SELECT DISTINCT a.i_docno FROM lan:serv_infdochd a, lan:serv_infdocdt b ")
	  .append("WHERE a.f_status = 'OPN' AND a.i_doc_type = 'J' "+team_condition)
	  .append(" AND b.i_itmtype = '02' AND b.f_itmstatus = '300' "+condition+" ")
	  .append(" AND b.i_docno = a.i_docno");
//System.out.println("---77777--"+sql.toString());	  
	rs = stmt.executeQuery(sql.toString());
	while (rs.next() == true) {
		completeWait++;
	} // end while
	...
	sql.append("SELECT COUNT(*) FROM lan:serv_infdochd a, lan:serv_infpayment b ")
		.append("WHERE a.f_status = 'OPN' AND a.i_doc_type = 'J' "+team_condition+" AND b.i_itmtype = '02' AND b.f_itmstatus = '400' ")
		.append("AND b.i_docno = a.i_docno ").append(condition)
		.append(" GROUP BY a.i_docno");
	completePass = countResultRow(stmt,sql.toString(),servlog);
...
/*
	...
	if (rs != null) {
		if (rs.next() == true) { //ALL Project
			sql.append("SELECT a.i_seq, a.i_chart_grp, a.f_approve, COUNT(DISTINCT h.i_docno) AS NUM_DOC FROM lan:serv_infdochd h, lan:serv_infdocap a, lan:serv_infdocdt d")
				.append(" WHERE h.i_docno = a.i_docno AND h.i_docno = d.i_docno AND d.i_itmtype = '02' AND h.f_status != 'CAN'")
				.append(inf_condition)
...
*/	
	team_condition = "";
	if (!user_group.equals("A")) {
		team_condition = " AND h.i_team = '"+user_group+"' ";
	}
	sql.delete(0,sql.length());
	if (!inf_condition.equals("")) {
		sql.append("SELECT a.i_seq, a.i_chart_grp, a.f_approve, COUNT(DISTINCT h.i_docno) AS NUM_DOC")
			.append(" FROM lan:serv_infdochd h, lan:serv_infdocdt d, lan:serv_infdocap a")
			.append(" WHERE h.i_docno = a.i_docno AND h.i_docno = d.i_docno AND d.i_itmtype = '02' AND h.f_status != 'CAN' ")
			.append(team_condition)
			.append(inf_condition)
			.append(" GROUP BY a.i_seq, a.i_chart_grp, a.f_approve ORDER BY a.i_seq, a.i_chart_grp, a.f_approve");
		...
		sql.append("SELECT a.i_seq, a.i_chart_grp, a.f_approve, COUNT(DISTINCT h.i_docno) AS NUM_DOC")
			.append(" FROM lan:serv_infdochd h, lan:serv_infdocdt d, lan:serv_infdocap a, lan:serv_pstaff s")
			.append(" WHERE s.user_id = '")
			.append(userId)
			.append("' AND s.com_id = h.i_company AND s.proj_id = h.i_project")
			.append(" AND h.i_docno = a.i_docno AND h.i_docno = d.i_docno AND d.i_itmtype = '02' AND h.f_status != 'CAN' ")
			.append(team_condition)
			.append(inf_condition)
			.append(" GROUP BY a.i_seq, a.i_chart_grp, a.f_approve ORDER BY a.i_seq, a.i_chart_grp, a.f_approve");
	}		
	...
	//System.out.println("999999 = "+sql.toString());
	rs = stmt.executeQuery(sql.toString());
	if (rs != null) {
		while (rs.next() == true) {
			...
			if (!curGroup.equals(lastGroup)) { //Change Chart Group
				if (!lastGroup.equals("")) {
					...
					if (lastGroup.equals("S")) {
						...
						desc = "Zone Manager - ผู้จัดการกลุ่ม";
					}
%>
                <tr> 
                  ...
                  <td width="15%" class="dotline" align="center"><a href="<%=targetPage%>?selProj=<%=selProj%>&chartGrp=<%=lastGroup%>&status=W&itmType=02"><%=jobWait%></a></td>
                  <td width="15%" class="dotline" align="center"><%=jobPass%></td>
                  <td width="15%" class="dotline" align="center">
                  <%if (lastGroup.equals("S")) {%><a href="<%=targetPage%>?selProj=<%=selProj%>&chartGrp=<%=lastGroup%>&status=R&itmType=02"><%}%>
                  <%=jobReject%>
                  </td>
                </tr>
<%				
	...
	if (!lastGroup.equals("")) {
		...
		if (lastGroup.equals("S")) {
			...
			desc = "Zone Manager - ผู้จัดการกลุ่ม อนุมัติงานสั่งซ่อม";
		}
%>
                <tr> 
                  <td width="55%" class="item ; dotline" align="left"><img border="0" src="images/i_arrow1.gif" align="absmiddle" width="13" height="13"><%=desc%></td>
...
%>                
                <tr> 
                  <td width="55%" class="item ; dotline" align="left"><img border="0" src="images/i_arrow1.gif" align="absmiddle" width="13" height="13"> 
                    Complete - ผู้รับเหมาปิดงาน</td>
                  <td width="15%" class="dotline" align="center"><a href="SERV_INFCompTask_List.jsp?sel_project=<%=selProj%>&itmType=02"><%=completeWait%></a></td>
                  <td width="15%" class="dotline" align="center"><%=completePass%></td>
                  <td width="15%" class="dotline" align="center">&nbsp;</td>
                </tr>
                <tr> 
...
<br style="font-size:10pt">
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          <td width="40%" class="bigh">&nbsp;</td>
            <td width="60%" align="right">
			<a href="/LHServ/ServOpenJob?actionMode=initForm&i_itmtype=01"><img border="0" src="images/icon_open_JopINF.gif" width="120" height="34"></a> 
            </td>
        </tr>
<br style="font-size:10pt">
      </table>
...
<%
	...
	if (viewAllStaffProj) {
		...
		sql.append(" SELECT DISTINCT b.i_docno FROM lan:serv_pstaff a,lan:serv_infdochd b, lan:serv_infdocdt c ")
			  ...
			  .append(" AND c.i_itmtype = '01' AND c.f_itmstatus = '300' AND c.i_docno = b.i_docno ")
			  .append(" AND a.user_id = '"+userId+"' AND a.com_id = b.i_company ")
			  .append(" AND a.proj_id = b.i_project ");
	} else {
	}
	...
	sql.append("SELECT DISTINCT a.i_docno FROM lan:serv_infdochd a, lan:serv_infdocdt b ")
	  ...
	  .append("AND b.i_itmtype = '01' AND b.f_itmstatus = '300' "+condition+" ")
	  .append(" AND b.i_docno = a.i_docno");
	//System.out.println("SQL 1111 = "+sql.toString());
	rs = stmt.executeQuery(sql.toString());
	while (rs.next() == true) {
		completeWait++;
	} // end while
	...
	if (viewAllStaffProj) {
		...
		sql.append(" SELECT COUNT(*) FROM lan:serv_pstaff a,lan:serv_infdochd b,lan:serv_infpayment c ")
			  .append(" WHERE b.f_status = 'OPN' AND b.i_doc_type = 'J' ")
			  .append(" AND c.i_itmtype = '01' AND c.f_itmstatus = '400' AND c.i_docno = b.i_docno ")
			  .append(" AND a.user_id = '"+userId+"' AND a.com_id = b.i_company ")
			  .append(" AND a.proj_id = b.i_project ")
			  .append(" GROUP BY b.i_docno ");
	} else {
	...
	sql.delete(0,sql.length());
	sql.append("SELECT COUNT(*) FROM lan:serv_infdochd a, lan:serv_infpayment b ")
		.append("WHERE a.f_status = 'OPN' AND a.i_doc_type = 'J' AND b.i_itmtype = '01' AND b.f_itmstatus = '400' ")
		.append("AND b.i_docno = a.i_docno ").append(condition)
		.append(" GROUP BY a.i_docno");
	...
	if (rs != null) {
		if (rs.next() == true) { //ALL Project
			sql.append("SELECT a.i_seq, a.i_chart_grp, a.f_approve, COUNT(DISTINCT h.i_docno) AS NUM_DOC FROM lan:serv_infdochd h, lan:serv_infdocap a, lan:serv_infdocdt d")
				.append(" WHERE h.i_docno = a.i_docno AND h.i_docno = d.i_docno AND d.i_itmtype = '01' AND h.f_status != 'CAN'")
				.append(inf_condition)
				.append(" GROUP BY a.i_seq, a.i_chart_grp, a.f_approve ORDER BY a.i_seq, a.i_chart_grp, a.f_approve");
		} else {
		}
	...
	if (inf_condition.equals("")) {
		sql.append("SELECT a.i_seq, a.i_chart_grp, a.f_approve, COUNT(DISTINCT h.i_docno) AS NUM_DOC")
			...
			.append(userId)
			.append("' AND s.com_id = h.i_company AND s.proj_id = h.i_project")
			.append(" AND h.i_docno = a.i_docno AND h.i_docno = d.i_docno AND d.i_itmtype = '01' AND h.f_status != 'CAN'")
			.append(inf_condition)
			.append(" GROUP BY a.i_seq, a.i_chart_grp, a.f_approve ORDER BY a.i_seq, a.i_chart_grp, a.f_approve");
	} else {
		sql.append("SELECT a.i_seq, a.i_chart_grp, a.f_approve, COUNT(DISTINCT h.i_docno) AS NUM_DOC")
	...
	jobPass = 0;
	jobReject = 0;
//System.out.println("SQL 33333 = "+sql.toString());
	rs = stmt.executeQuery(sql.toString());
	if (rs != null) {
		while (rs.next() == true) {
			curGroup = doString.checkString(rs.getString("I_CHART_GRP"));
...
%>
                <tr> 
                  <td width="55%" class="item ; dotline" align="left"><img border="0" src="images/i_arrow1.gif" align="absmiddle" width="13" height="13"><%=desc%></td>
                  <td width="15%" class="dotline" align="center"><a href="<%=targetPage%>?selProj=<%=selProj%>&chartGrp=<%=lastGroup%>&status=W&itmType=01"><%=jobWait%></a></td>
                  <td width="15%" class="dotline" align="center"><%=jobPass%></td>
                  <td width="15%" class="dotline" align="center">
                  <%if (lastGroup.equals("S")) {%><a href="<%=targetPage%>?selProj=<%=selProj%>&chartGrp=<%=lastGroup%>&status=R&itmType=01"><%}%>
                  <%=jobReject%>
                  </td>
                </tr>
<%				
...
%>                
                <tr> 
                  <td width="55%" class="item ; dotline" align="left"><img border="0" src="images/i_arrow1.gif" align="absmiddle" width="13" height="13"> 
                    Complete - ผู้รับเหมาปิดงาน</td>
                  <td width="15%" class="dotline" align="center"><a href="SERV_INFCompTask_List.jsp?sel_project=<%=selProj%>&itmType=01"><%=completeWait%></a></td>
                  <td width="15%" class="dotline" align="center"><%=completePass%></td>
                  <td width="15%" class="dotline" align="center">&nbsp;</td>
                </tr>
                <tr> 
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td width="7%" class="dotline" align="center"><a href="SERV_Pass_List.jsp?sel_project=<%=selProj%>&d_payment=<%=showCurrentPaymentDate%>"><%=cVPPass%></a></td>
    <td width="7%" class="dotline" align="center">&nbsp;</td>
...
<%
       ...
       System.out.println("--------- SERV_Home.jsp ---------");
	} catch (Exception e) {
